...
)
{
	...
	if (NULL == macEntry)
	{
		MSD_DBG_ERROR(("Input param MSD_ATU_ENTRY in Agate_gfdbAddMacEntryIntf is NULL. \n"));
    ...
    return Agate_gfdbAddMacEntry(dev, &entry);
}
...
)
{
	...
	if (NULL == atuEntry)
	{
		MSD_DBG_ERROR(("Input param MSD_ATU_ENTRY in Agate_gfdbGetAtuEntryNextIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
...
{
	return Agate_gfdbFlush(dev, (AGATE_MSD_FLUSH_CMD)flushCmd);
}
...
{
	return Agate_gfdbFlushInDB(dev, (AGATE_MSD_FLUSH_CMD)flushCmd, DBNum);
}
...
{
	return Agate_gfdbMove(dev, (AGATE_MSD_MOVE_CMD)moveCmd, moveFrom, moveTo);
}
...
{
	return Agate_gfdbMoveInDB(dev, (AGATE_MSD_MOVE_CMD)moveCmd, DBNum, moveFrom, moveTo);
}
...
{
    AGATE_MSD_MOVE_CMD pmoveCmd;

    switch (moveCmd)
    {
    case MSD_MOVE_ALL:
        pmoveCmd = AGATE_MSD_MOVE_ALL;
        break;
    case MSD_MOVE_ALL_NONSTATIC:
        pmoveCmd = AGATE_MSD_MOVE_ALL_NONSTATIC;
        break;
    default:
        return MSD_BAD_PARAM;
    }

    return Agate_gfdbMove(dev, pmoveCmd, portNum, 0x1F);
}
...
)
{
    ...
    return Agate_gfdbMoveInDB(dev, pmoveCmd, DBNum, portNum, 0x1F);
}
...
{
	AGATE_MSD_ATU_INT_STATUS atuInt;
	MSD_STATUS       retVal;

	msdMemSet((void*)(&atuInt), 0, sizeof(AGATE_MSD_ATU_INT_STATUS));
	retVal = Agate_gfdbGetViolation(dev, &atuInt);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == atuIntStatus)
	{
		MSD_DBG_ERROR(("Input param MSD_ATU_INT_STATUS in Agate_gfdbGetViolationIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
	msdMemSet((void*)atuIntStatus, 0, sizeof(MSD_ATU_INT_STATUS));
    atuIntStatus->atuIntCause.ageOutVio = ((atuInt.atuIntCause & AGATE_MSD_ATU_AGE_OUT_VIOLATION) == 0) ? MSD_FALSE : MSD_TRUE;
    atuIntStatus->atuIntCause.memberVio = ((atuInt.atuIntCause & AGATE_MSD_ATU_MEMBER_VIOLATION) == 0) ? MSD_FALSE : MSD_TRUE;
    atuIntStatus->atuIntCause.missVio = ((atuInt.atuIntCause & AGATE_MSD_ATU_MISS_VIOLATION) == 0) ? MSD_FALSE : MSD_TRUE;
    atuIntStatus->atuIntCause.fullVio = ((atuInt.atuIntCause & AGATE_MSD_ATU_FULL_VIOLATION) == 0) ? MSD_FALSE : MSD_TRUE;
...
)
{
    ...
    if (NULL == macAddr)
    {
        MSD_DBG_ERROR(("Input param MSD_ETHERADDR in Agate_gfdbFindAtuMacEntryIntf is NULL. \n"));
        return MSD_BAD_PARAM;
    }
    
    msdMemSet((void*)(&entry), 0, sizeof(AGATE_MSD_ATU_ENTRY));
    entry.DBNum = (MSD_U16)fid;
	msdMemCpy(entry.macAddr.arEther, macAddr, 6);

	retVal = Agate_gfdbFindAtuMacEntry(dev, &entry, found);
	if (MSD_OK != retVal || MSD_FALSE == *found)
		return retVal;

    if (NULL == atuEntry)
    {
        MSD_DBG_ERROR(("Input param MSD_ATU_ENTRY in Agate_gfdbFindAtuMacEntryIntf is NULL. \n"));
        return MSD_BAD_PARAM;
    }
	msdMemSet((void*)atuEntry, 0, sizeof(MSD_ATU_ENTRY));
	atuEntry->fid = entry.DBNum;
...
)
{
    ...
    while (1 == temp)
    {
        status = Agate_gfdbGetAtuEntryNextIntf(dev, &entry);
