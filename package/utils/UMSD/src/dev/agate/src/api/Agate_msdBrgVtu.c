...
static MSD_STATUS Agate_vtuOperationPerform
(
IN        MSD_QD_DEV           *dev,
IN      AGATE_MSD_VTU_OPERATION    vtuOp,
INOUT   MSD_U8               *valid,
...
)
{
	...
	MSD_DBG_INFO(("Agate_gvtuGetEntryNext Called.\n"));
	...
	retVal = Agate_vtuOperationPerform(dev, AGATE_GET_NEXT_ENTRY, &valid, &entry);
	if (retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Agate_vtuOperationPerform AGATE_GET_NEXT_ENTRY returned: %s.\n", msdDisplayStatus(retVal)));
		return retVal;
	}
	...
	{
		MSD_DBG_INFO(("Agate_gvtuGetEntryNext Exit, MSD_NO_SUCH\n"));
		return MSD_NO_SUCH;
	}
	...
	MSD_DBG_INFO(("Agate_gvtuGetEntryNext Exit.\n"));
	return MSD_OK;
}
...
)
{
	...
	MSD_DBG_INFO(("Agate_gvtuFlush Called.\n"));

	retVal = Agate_vtuOperationPerform(dev, AGATE_FLUSH_ALL, NULL, NULL);
	if (retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Agate_vtuOperationPerform AGATE_FLUSH_ALL returned: %d.\n", msdDisplayStatus(retVal)));
		return retVal;
	}

	MSD_DBG_INFO(("Agate_gvtuFlush Exit.\n"));
	return MSD_OK;
}
...
)
{
	...
	MSD_DBG_INFO(("Agate_gvtuAddEntry Called.\n"));
	...
	for (port = 0; port<dev->maxPorts; port++)
	{
		...
		{
			entry.memberTagP[port] = Agate_NOT_A_MEMBER;
		}
		else
		{
	...
	retVal = Agate_vtuOperationPerform(dev, AGATE_LOAD_PURGE_ENTRY, &valid, &entry);
	if (retVal != MSD_OK)
	{
	...
	MSD_DBG_INFO(("Agate_gvtuAddEntry Exit.\n"));
	return MSD_OK;
}
...
)
{
	...
	MSD_DBG_INFO(("Agate_gvtuDelEntry Called.\n"));
	...
	if (retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Agate_vtuOperationPerform delete entry returned: %d.\n", msdDisplayStatus(retVal)));
		return retVal;
	}
	MSD_DBG_INFO(("Agate_gvtuDelEntry Exit.\n"));
	return MSD_OK;
}
...
)
{
	...
	MSD_DBG_INFO(("Agate_gvtuGetViolation Called.\n"));

	/* check Violation occurred */
	retVal = msdGetAnyRegField(dev->devNum, AGATE_GLOBAL1_DEV_ADDR, AGATE_QD_REG_GLOBAL_STATUS, 5, 1, &intCause);
	if (retVal != MSD_OK)
	{
	...
	if (intCause == 0)
	{
		...
		MSD_DBG_INFO(("Agate_gvtuGetViolation Exit, No Violation occurred.\n"));
		return MSD_OK;
	}

	entry.DBNum = 0;

	retVal = Agate_vtuOperationPerform(dev, AGATE_SERVICE_VIOLATIONS, NULL, &entry);
	if (retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Agate_vtuOperationPerform AGATE_SERVICE_VIOLATIONS returned: %s.\n", msdDisplayStatus(retVal)));
		return retVal;
	}

	retVal = msdGetAnyRegField(dev->devNum, AGATE_GLOBAL1_DEV_ADDR, AGATE_QD_REG_VTU_OPERATION, 4, 3, &intCause);
	if (retVal != MSD_OK)
	{
	...
	if (spid == 0xF)
	{
		vtuIntStatus->vtuIntCause = AGATE_MSD_VTU_FULL_VIOLATION;
		vtuIntStatus->spid = spid;
		vtuIntStatus->vid = 0;
	}
	else
	{
		vtuIntStatus->vtuIntCause = intCause & (AGATE_MSD_VTU_MEMBER_VIOLATION | AGATE_MSD_VTU_MISS_VIOLATION);
		vtuIntStatus->spid = spid;
		vtuIntStatus->vid = vid;
	}

	MSD_DBG_INFO(("Agate_gvtuGetViolation Exit.\n"));
	return MSD_OK;
}
...
)
{
	...
	MSD_DBG_INFO(("Agate_gvtuFindVidEntry Called.\n"));
	...
	*found = MSD_FALSE;
	...
	if (retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Agate_vtuOperationPerform vtuFindVidEntry returned: %d).\n", msdDisplayStatus(retVal)));
		return retVal;
	}
	...
	{
		MSD_DBG_INFO(("Agate_gvtuFindVidEntry Exit, MSD_NO_SUCH\n"));
		return MSD_OK;
	}
	...
	*found = MSD_TRUE;
	...
	MSD_DBG_INFO(("Agate_gvtuFindVidEntry Exit.\n"));
	return MSD_OK;
}
...
IN      AGATE_MSD_VTU_OPERATION    vtuOp,
INOUT   MSD_U8               *valid,
INOUT    AGATE_MSD_VTU_ENTRY        *entry
)
{
	...
	if ((vtuOp != AGATE_FLUSH_ALL) && (vtuOp != AGATE_GET_NEXT_ENTRY) && (vtuOp != AGATE_SERVICE_VIOLATIONS))
	{
		/****************** VTU DATA 1 REG *******************/
		...
		retVal = msdSetAnyReg(dev->devNum, AGATE_GLOBAL1_DEV_ADDR, AGATE_QD_REG_VTU_DATA1_REG, data);
		if (retVal != MSD_OK)
		{
			msdSemGive(dev->devNum, dev->vtuRegsSem);
			return retVal;
		...
		if (dev->maxPorts > 4)
		{
			...
			retVal = msdSetAnyReg(dev->devNum, AGATE_GLOBAL1_DEV_ADDR, AGATE_QD_REG_VTU_DATA2_REG, data);
			if (retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->vtuRegsSem);
				return retVal;
		...
		retVal = msdSetAnyReg(dev->devNum, AGATE_GLOBAL1_DEV_ADDR, AGATE_QD_REG_VTU_DATA3_REG, data);
		if (retVal != MSD_OK)
		{
			msdSemGive(dev->devNum, dev->vtuRegsSem);
			return retVal;
	...
	if ((vtuOp != AGATE_FLUSH_ALL) && (vtuOp != AGATE_SERVICE_VIOLATIONS))
	{
		data = 0;
	...
	if ((vtuOp == AGATE_LOAD_PURGE_ENTRY) && (*valid == 1))
	{
		data = (entry->sid) & 0x3F;		
		...
		retVal = msdSetAnyReg(dev->devNum, AGATE_GLOBAL1_DEV_ADDR, (MSD_U8)(AGATE_QD_REG_VTU_FID_REG), data);
		if (retVal != MSD_OK)
		{
			msdSemGive(dev->devNum, dev->vtuRegsSem);
			return retVal;
	...
	*/
	...
	if (vtuOp == AGATE_SERVICE_VIOLATIONS)
	{
		...
		retVal = msdGetAnyRegField(dev->devNum, AGATE_GLOBAL1_DEV_ADDR, AGATE_QD_REG_VTU_OPERATION, 0, 4, &data);
		...
		retVal = msdGetAnyReg(dev->devNum, AGATE_GLOBAL1_DEV_ADDR, AGATE_QD_REG_VTU_VID_REG, &data);
		if (retVal != MSD_OK)
		{
			msdSemGive(dev->devNum, dev->vtuRegsSem);
			return retVal;
	...
	if (vtuOp == AGATE_GET_NEXT_ENTRY)
	{
		...
		*valid = (MSD_U8)((data >> 12) & 1);
		...
		/****************** get the SID *******************/
		retVal = msdGetAnyReg(dev->devNum, AGATE_GLOBAL1_DEV_ADDR, (MSD_U8)(AGATE_QD_REG_STU_SID_REG), &data);
		if (retVal != MSD_OK)
		{
			msdSemGive(dev->devNum, dev->vtuRegsSem);
			return retVal;
		...
		retVal = msdGetAnyReg(dev->devNum, AGATE_GLOBAL1_DEV_ADDR, (MSD_U8)(AGATE_QD_REG_VTU_FID_REG), &data);
		if (retVal != MSD_OK)
		{
			msdSemGive(dev->devNum, dev->vtuRegsSem);
			return retVal;
		...
		retVal = msdGetAnyReg(dev->devNum, AGATE_GLOBAL1_DEV_ADDR, AGATE_QD_REG_VTU_DATA1_REG, &data);
		if (retVal != MSD_OK)
		{
			msdSemGive(dev->devNum, dev->vtuRegsSem);
			return retVal;
		...
		if (dev->maxPorts > 3)
		{
			...
			retVal = msdGetAnyReg(dev->devNum, AGATE_GLOBAL1_DEV_ADDR, AGATE_QD_REG_VTU_DATA2_REG, &data);
			if (retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->vtuRegsSem);
				return retVal;
		...
		/* get data from data register for ports 4 to 6 */
		retVal = msdGetAnyReg(dev->devNum, AGATE_GLOBAL1_DEV_ADDR, AGATE_QD_REG_VTU_DATA3_REG, &data);
		if (retVal != MSD_OK)
		{
			msdSemGive(dev->devNum, dev->vtuRegsSem);
			return retVal;
...
)
{
	...
	msdMemSet(&entry, 0, sizeof(AGATE_MSD_VTU_ENTRY));
	entry.vid = 0xfff;
	...
	while (1 == temp)
	{
		status = Agate_gvtuGetEntryNext(dev, &entry);
