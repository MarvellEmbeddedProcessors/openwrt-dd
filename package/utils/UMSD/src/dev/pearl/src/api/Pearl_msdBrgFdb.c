...
typedef enum
{
    PEARL_MSD_ATU_STATS_ALL = 0,
    PEARL_MSD_ATU_STATS_NON_STATIC,
    PEARL_MSD_ATU_STATS_ALL_FID,
    PEARL_MSD_ATU_STATS_NON_STATIC_FID
} PEARL_MSD_ATU_STATS_OP;
...
/****************************************************************************/
static MSD_STATUS Pearl_atuOperationPerform
(
    IN      MSD_QD_DEV           *dev,
    IN      PEARL_MSD_ATU_OPERATION    atuOp,
    INOUT   PEARL_MSD_EXTRA_OP_DATA    *opData,
    INOUT   PEARL_MSD_ATU_ENTRY        *atuEntry
);

static MSD_STATUS Pearl_atuGetStats
(
IN  MSD_QD_DEV    *dev,
IN  PEARL_MSD_ATU_STAT    *atuStat,
OUT MSD_U32        *count
);
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetAtuEntryNext Called.\n"));
    ...
    retVal = Pearl_atuOperationPerform(dev,PEARL_GET_NEXT_ENTRY,NULL,&entry);
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Pearl_atuOperationPerform PEARL_GET_NEXT_ENTRY returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    if(PEARL_IS_BROADCAST_MAC(entry.macAddr))
    {
        if(entry.entryState == 0)
        {
            ...
            MSD_DBG_INFO(("Pearl_gfdbGetAtuEntryNext Exit.\n"));
            return MSD_NO_SUCH;
        }
    }
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbFlush Called.\n"));

    entry.DBNum = 0;
    entry.entryState = 0;

    if(flushCmd == PEARL_MSD_FLUSH_ALL)
        retVal = Pearl_atuOperationPerform(dev,PEARL_FLUSH_ALL,NULL,&entry);
    else if(flushCmd == PEARL_MSD_FLUSH_ALL_NONSTATIC)
        retVal = Pearl_atuOperationPerform(dev,PEARL_FLUSH_NONSTATIC,NULL,&entry);
    else
    {
        MSD_DBG_ERROR(("Bad Param: PEARL_MSD_FLUSH_CMD is %d\n", flushCmd));
        return MSD_FAIL;
    }

    if(retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Pearl_atuOperationPerform PEARL_FLUSH returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Pearl_gfdbFlush Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbFlushInDB Called.\n"));
    ...
    if(flushCmd == PEARL_MSD_FLUSH_ALL)
        retVal = Pearl_atuOperationPerform(dev,PEARL_FLUSH_ALL_IN_DB,NULL,&entry);
    else if(flushCmd == PEARL_MSD_FLUSH_ALL_NONSTATIC)
        retVal = Pearl_atuOperationPerform(dev,PEARL_FLUSH_NONSTATIC_IN_DB,NULL,&entry);
    else
    {
        MSD_DBG_ERROR(("Bad Param: PEARL_MSD_FLUSH_CMD is %d\n", flushCmd));
        return MSD_FAIL;
    ...
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Pearl_atuOperationPerform PEARL_FLUSH_IN_DB returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Pearl_gfdbFlushInDB Exit.\n"));
    return MSD_OK;
}
...
)
{
    MSD_STATUS       retVal;
    PEARL_MSD_ATU_ENTRY    entry;
    PEARL_MSD_EXTRA_OP_DATA    opData;

    MSD_DBG_INFO(("Pearl_gfdbMove Called.\n"));
    ...
    if(moveCmd == PEARL_MSD_MOVE_ALL)
        retVal = Pearl_atuOperationPerform(dev,PEARL_FLUSH_ALL,&opData,&entry);
    else if (moveCmd == PEARL_MSD_MOVE_ALL_NONSTATIC)
        retVal = Pearl_atuOperationPerform(dev,PEARL_FLUSH_NONSTATIC,&opData,&entry);
    else
    {
        MSD_DBG_ERROR(("Bad Param: PEARL_MSD_MOVE_CMD is %d\n", moveCmd));
        return MSD_FAIL;
    }

    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Pearl_atuOperationPerform PEARL_MOVE returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Pearl_gfdbMove Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbMoveInDB Called.\n"));
    ...
    if(moveCmd == PEARL_MSD_MOVE_ALL)
        retVal = Pearl_atuOperationPerform(dev,PEARL_FLUSH_ALL_IN_DB,&opData,&entry);
    else if (moveCmd == PEARL_MSD_MOVE_ALL_NONSTATIC)
        retVal = Pearl_atuOperationPerform(dev,PEARL_FLUSH_NONSTATIC_IN_DB,&opData,&entry);
    else
    {
        MSD_DBG_ERROR(("Bad Param: PEARL_MSD_MOVE_CMD is %d\n", moveCmd));
        return MSD_FAIL;
    ...
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Pearl_atuOperationPerform PEARL_MOVE_IN_DB returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Pearl_gfdbMoveInDB Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbAddMacEntry Called.\n"));
    ...
    if (MSD_TRUE == macEntry->trunkMember && 0 == PEARL_IS_TRUNK_ID_VALID(dev, macEntry->portVec))
    {
    ...
    retVal = Pearl_atuOperationPerform(dev,PEARL_LOAD_PURGE_ENTRY,NULL,&entry);
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Pearl_atuOperationPerform PEARL_LOAD_PURGE_ENTRY returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Pearl_gfdbAddMacEntry Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbDelAtuEntry Called.\n"));

    if (NULL == macAddr)
    {
        MSD_DBG_ERROR(("Input param MSD_ETHERADDR in Pearl_gfdbDelAtuEntry is NULL. \n"));
        return MSD_BAD_PARAM;
    }
    ...
    MSD_DBG_INFO(("Pearl_gfdbDelAtuEntry Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetViolation Called.\n"));

    /* check which Violation occurred */
    retVal = msdGetAnyRegField(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_GLOBAL_STATUS,3,1,&intCause);
    if(retVal != MSD_OK)
    {
    ...
    if (!intCause)
    {
		...
		MSD_DBG_INFO(("Pearl_gfdbGetViolation Exit, No Violation occurred.\n"));
        return MSD_OK;
    }

    entry.DBNum = 0;

    retVal = Pearl_atuOperationPerform(dev,PEARL_SERVICE_VIOLATIONS,&opData,&entry);
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Pearl_atuOperationPerform PEARL_SERVICE_VIOLATIONS returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetViolation Exit.\n"));
    return MSD_OK;
}
...
)
{
	...
	MSD_DBG_INFO(("Pearl_gfdbFindAtuMacEntry Called.\n"));
	...
	*found = MSD_FALSE;
    ...
    {
        MSD_DBG_INFO(("Pearl_gfdbFindAtuMacEntry Exit, Not found valid entry.\n"));
        return MSD_OK;
    }

	if (retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Pearl_gfdbGetAtuEntryNext returned: %s.\n", msdDisplayStatus(retVal)));
		return retVal;
	}
	...
	{
        MSD_DBG_INFO(("Pearl_gfdbFindAtuMacEntry Exit, Not found this entry.\n"));
        return MSD_OK;
	}
	...
	*found = MSD_TRUE;
	MSD_DBG_INFO(("Pearl_gfdbFindAtuMacEntry Exit.\n"));
	return MSD_OK;
}
...
*        For Pearl, the time-base is 15000 milliseconds, so it can be set to 15000,
...
*        15000 will be used instead. E.g.) 1400 becomes 15000.
*
* INPUTS:
...
)
{
	...
	MSD_DBG_INFO(("Pearl_gfdbSetAgingTimeout Called.\n"));
	...
	retVal = msdSetAnyRegField(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR, PEARL_QD_REG_ATU_CTRL_REG, 4, 8, data);
	if (retVal != MSD_OK)
	{
	...
	MSD_DBG_INFO(("Pearl_gfdbSetAgingTimeout Exit.\n"));
	return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetAgingTimeout Called.\n"));

	timeBase = 15000; /*timeBase unit is millisecond*/

    /* Set the Time Out value.              */
    retVal = msdGetAnyRegField(dev->devNum, PEARL_GLOBAL1_DEV_ADDR, PEARL_QD_REG_ATU_CTRL_REG, 4, 8, &data);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdSetAnyRegField returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    ...
    *timeout = (MSD_U32)(data*timeBase);
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetAgingTimeout Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetLearn2All Called.\n"));

    /* Get the Learn2All. */
    data = 0;
    retVal = msdGetAnyRegField(dev->devNum, PEARL_GLOBAL1_DEV_ADDR, PEARL_QD_REG_ATU_CTRL_REG, 3, 1, &data);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdGetAnyRegField for PEARL_QD_REG_ATU_CTRL_REG returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_BIT_2_BOOL(data, *mode);

    MSD_DBG_INFO(("Pearl_gfdbGetLearn2All Exit.\n"));
    return retVal;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbSetLearn2All Called.\n"));

    /* Set the Learn2All. */
    MSD_BOOL_2_BIT(mode, data);
    retVal = msdSetAnyRegField(dev->devNum, PEARL_GLOBAL1_DEV_ADDR, PEARL_QD_REG_ATU_CTRL_REG, 3, 1, data);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdSetAnyRegField for PEARL_QD_REG_ATU_CTRL_REG returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Pearl_gfdbSetLearn2All Exit.\n"));
    return retVal;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbSetPortLearnLimit Called.\n"));
    ...
    if (limit > PEARL_MAX_ATU_PORT_LEARNLIMIT)
    {
        MSD_DBG_ERROR(("Learn Limit %u our of range. It should be within [0, %u].\n", (unsigned int)limit, PEARL_MAX_ATU_PORT_LEARNLIMIT));
        return MSD_BAD_PARAM;
    }
    ...
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdGetAnyRegField for PEARL_QD_REG_PORT_CONTROL1 returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    retVal = msdGetAnyReg(dev->devNum, phyAddr, PEARL_QD_REG_PORT_ATU_CONTROL, &data);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdGetAnyReg for PEARL_QD_REG_PORT_ATU_CONTROL returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    data &= 0x6000;
    data |= limit;
    retVal = msdSetAnyReg(dev->devNum, phyAddr, PEARL_QD_REG_PORT_ATU_CONTROL, data);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdSetAnyReg for PEARL_QD_REG_PORT_ATU_CONTROL returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    /* Set back KeepOldLearnLimit */
    retVal = msdSetAnyRegField(dev->devNum, phyAddr, PEARL_QD_REG_PORT_ATU_CONTROL, 12, 1, (MSD_U16)isKeepOldLearnLimit);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdSetAnyRegField for PEARL_QD_REG_PORT_ATU_CONTROL returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Pearl_gfdbSetPortLearnLimit Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetPortLearnLimit Called.\n"));
    ...
    /* Get back limit*/
    retVal = msdGetAnyRegField(dev->devNum, phyAddr, PEARL_QD_REG_PORT_ATU_CONTROL, 0, 10, &data);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdGetAnyRegField for PEARL_QD_REG_PORT_ATU_CONTROL returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    *limit = data;
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetPortLearnLimit Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetPortLearnCount Called.\n"));
	...
	retVal = msdSetAnyRegField(dev->devNum, phyAddr, PEARL_QD_REG_PORT_ATU_CONTROL, 15, 1, 1);
	if (retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("msdSetAnyRegField for PEARL_QD_REG_PORT_ATU_CONTROL returned: %s.\n", msdDisplayStatus(retVal)));
		return retVal;
    ...
    *count = data;
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetPortLearnCount Exit.\n"));
    return MSD_OK;
}
...
{
    MSD_STATUS       retVal;
    PEARL_MSD_ATU_STAT        atuStat;

    MSD_DBG_INFO(("Pearl_gfdbGetEntryCount Called.\n"));

    atuStat.op = PEARL_MSD_ATU_STATS_ALL;
    retVal = Pearl_atuGetStats(dev, &atuStat, count);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Pearl_atuGetStats returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Pearl_gfdbGetEntryCount Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetEntryCountPerFid Called.\n"));

    atuStat.op = PEARL_MSD_ATU_STATS_ALL_FID;
    atuStat.DBNum = fid;
    retVal = Pearl_atuGetStats(dev, &atuStat, count);
    if (retVal != MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetEntryCountPerFid Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetNonStaticEntryCount Called.\n"));

    atuStat.op = PEARL_MSD_ATU_STATS_NON_STATIC;
    retVal = Pearl_atuGetStats(dev, &atuStat, count);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Pearl_atuGetStats returned: %s.\n", msdDisplayStatus(retVal)));
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetNonStaticEntryCount Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetNonStaticEntryCountPerFid Called.\n"));

    atuStat.op = PEARL_MSD_ATU_STATS_NON_STATIC_FID;
    atuStat.DBNum = fid;
    retVal = Pearl_atuGetStats(dev, &atuStat, count);
    if (retVal != MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Pearl_gfdbGetNonStaticEntryCountPerFid Exit.\n"));
    return MSD_OK;
}
...
(
    ...
    IN       PEARL_MSD_ATU_OPERATION    atuOp,
    INOUT    PEARL_MSD_EXTRA_OP_DATA    *opData,
    INOUT    PEARL_MSD_ATU_ENTRY        *entry
)
{
    MSD_STATUS       retVal;         /* Functions return value.      */
    ...
    while(data == 1)
    {
        retVal = msdGetAnyRegField(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_ATU_OPERATION,15,1,&data);
        if(retVal != MSD_OK)
        {
    ...
    switch (atuOp)
    {
        case PEARL_LOAD_PURGE_ENTRY:
			...
			retVal = msdSetAnyReg(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_ATU_DATA_REG,data);
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->atuRegsSem);
				return retVal;
        ...
        case PEARL_GET_NEXT_ENTRY:
			for(i = 0; i < 3; i++)
			{
				data=(entry->macAddr.arEther[2*i] << 8)|(entry->macAddr.arEther[1 + 2*i]);
				retVal = msdSetAnyReg(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,(MSD_U8)(PEARL_QD_REG_ATU_MAC_BASE+i),data);
				if(retVal != MSD_OK)
				{
					msdSemGive(dev->devNum, dev->atuRegsSem);
					return retVal;
        ...
        case PEARL_FLUSH_ALL:
        case PEARL_FLUSH_NONSTATIC:
        case PEARL_FLUSH_ALL_IN_DB:
        case PEARL_FLUSH_NONSTATIC_IN_DB:
    ...
    /* Set DBNum */
    retVal = msdSetAnyRegField(dev->devNum, PEARL_GLOBAL1_DEV_ADDR, PEARL_QD_REG_ATU_FID_REG, 0, 12, (MSD_U16)(entry->DBNum & 0xFFF));
	if(retVal != MSD_OK)
	{
		msdSemGive(dev->devNum, dev->atuRegsSem);
		return retVal;
	...
	retVal = msdGetAnyReg(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_ATU_OPERATION,&data);
	if(retVal != MSD_OK)
	{
		msdSemGive(dev->devNum, dev->atuRegsSem);
		return retVal;
	...
	data &= 0x0fff;
	if(atuOp == PEARL_LOAD_PURGE_ENTRY)
	{
		data &= 0x0ff;
	}
	opcodeData |= ((1 << 15) | (atuOp << 12) | data);

    retVal = msdSetAnyReg(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_ATU_OPERATION,opcodeData);
    if(retVal != MSD_OK)
    {
        msdSemGive(dev->devNum, dev->atuRegsSem);
        return retVal;
    ...
    if(atuOp == PEARL_SERVICE_VIOLATIONS)
    {
        ...
        retVal = msdGetAnyRegField(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_ATU_OPERATION,4,4,&data);
        if(retVal != MSD_OK)
        {
            msdSemGive(dev->devNum, dev->atuRegsSem);
            return retVal;
        ...
        switch (data)
        {
            case 8:    /* Age Interrupt */
                opData->intCause = PEARL_MSD_ATU_AGE_OUT_VIOLATION;
                break;
            case 4:    /* Member Violation */
                opData->intCause = PEARL_MSD_ATU_MEMBER_VIOLATION;
                break;
            case 2:    /* Miss Violation */
                opData->intCause = PEARL_MSD_ATU_MISS_VIOLATION;
                break;
            case 1:    /* Full Violation */
                opData->intCause = PEARL_MSD_ATU_FULL_VIOLATION;
                break;
            default:
                opData->intCause = 0;
                msdSemGive(dev->devNum, dev->atuRegsSem);
		...
		retVal = msdGetAnyRegField(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_ATU_FID_REG,0,8,&data);
		if(retVal != MSD_OK)
		{
			msdSemGive(dev->devNum, dev->atuRegsSem);
			return retVal;
        ...
        retVal = msdGetAnyReg(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_ATU_DATA_REG,&data);
        if(retVal != MSD_OK)
        {
            msdSemGive(dev->devNum, dev->atuRegsSem);
            return retVal;
        ...
        for(i = 0; i < 3; i++)
        {
            retVal = msdGetAnyReg(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,(MSD_U8)(PEARL_QD_REG_ATU_MAC_BASE+i),&data);
            if(retVal != MSD_OK)
            {
                msdSemGive(dev->devNum, dev->atuRegsSem);
                return retVal;
    ...
    if(atuOp == PEARL_GET_NEXT_ENTRY)
    {
		...
		entry->entryState = (MSD_U8)(data & 0xF);
		retVal = msdGetAnyRegField(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_ATU_OPERATION,8,3,&data);
		if(retVal != MSD_OK)
		{
			msdSemGive(dev->devNum, dev->atuRegsSem);
			return retVal;
...
)
{
    ...
    switch (atuStat->op)
    {
    case PEARL_MSD_ATU_STATS_ALL:
    case PEARL_MSD_ATU_STATS_NON_STATIC:
        dbNum = 0;
        break;
    case PEARL_MSD_ATU_STATS_ALL_FID:
    case PEARL_MSD_ATU_STATS_NON_STATIC_FID:
        dbNum = atuStat->DBNum;
        break;
    default:
        return MSD_FAIL;
    ...
    for (bin = 0; bin<4; bin++)
    {
        ...
        retVal = msdSetAnyReg(dev->devNum, PEARL_GLOBAL2_DEV_ADDR, PEARL_QD_REG_ATU_STATS, data);
        if (retVal != MSD_OK)
        {
            return retVal;
        }
        ...
        retVal = msdSetAnyRegField(dev->devNum, PEARL_GLOBAL2_DEV_ADDR, PEARL_QD_REG_ATU_STATS, 14, 2, bin);
        if (retVal != MSD_OK)
        {
            return retVal;
        }

        retVal = msdGetAnyRegField(dev->devNum, PEARL_GLOBAL2_DEV_ADDR, PEARL_QD_REG_ATU_STATS, 0, 12, &data);
        if (retVal != MSD_OK)
        {
            return retVal;
        }
