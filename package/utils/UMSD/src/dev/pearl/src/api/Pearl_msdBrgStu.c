...
typedef enum
{
    PEARL_LOAD_PURGE_STU_ENTRY = 5,
    PEARL_GET_NEXT_STU_ENTRY =6 
} PEARL_MSD_STU_OPERATION;

static MSD_STATUS Pearl_stuOperationPerform
(
    IN        MSD_QD_DEV           *dev,
    IN      PEARL_MSD_STU_OPERATION    stuOp,
    INOUT   MSD_U8               *valid,
...
)
{
	...
	MSD_DBG_INFO(("Pearl_gstuGetEntryNext Called.\n"));
	...
	retVal = Pearl_stuOperationPerform(dev, PEARL_GET_NEXT_STU_ENTRY, &valid, &entry);
	if (retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Pearl_stuOperationPerform PEARL_GET_NEXT_STU_ENTRY returned: %s.\n", msdDisplayStatus(retVal)));
		return retVal;
	}
	...
	{
		MSD_DBG_INFO(("Pearl_gstuGetEntryNext Exit, MSD_NO_SUCH\n"));
		return MSD_NO_SUCH;
	}
    ...
    MSD_DBG_INFO(("Pearl_gstuGetEntryNext Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gstuAddEntry Called.\n"));
    ...
    retVal = Pearl_stuOperationPerform(dev,PEARL_LOAD_PURGE_STU_ENTRY,&valid, &entry);
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Pearl_stuOperationPerform add entry returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
	    
    MSD_DBG_INFO(("Pearl_gstuAddEntry Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gstuDelEntry Called.\n"));
    ...
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Pearl_stuOperationPerform Delete entry returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    MSD_DBG_INFO(("Pearl_gstuDelEntry Exit.\n"));
    return MSD_OK;
}
...
static MSD_STATUS Pearl_stuSetSTUData
(
    IN    MSD_QD_DEV           *dev,
    IN    PEARL_MSD_STU_ENTRY        *entry
)
{
    ...
    {
        case 3:
            retVal = msdSetAnyReg(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_VTU_DATA3_REG,data3);
            if(retVal != MSD_OK)
            {
                return retVal;
            }
            /* pass through */
        case 2:
            retVal = msdSetAnyReg(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_VTU_DATA2_REG,data2);
            if(retVal != MSD_OK)
            {
                return retVal;
            }
            /* pass through */
        case 1:
            retVal = msdSetAnyReg(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_VTU_DATA1_REG,data1);
            if(retVal != MSD_OK)
            {
                return retVal;
            }
...
static MSD_STATUS Pearl_stuGetSTUData
(
    IN     MSD_QD_DEV           *dev,
    OUT    PEARL_MSD_STU_ENTRY        *entry
)
{
    MSD_STATUS       retVal;         /* Functions return value.      */
    MSD_U16          data1,data2,data3;           /* Data to be set into the      */
    ...
    {
        case 3:
            retVal = msdGetAnyReg(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_VTU_DATA3_REG,&data3);
            if(retVal != MSD_OK)
            {
                return retVal;
            }
            /* pass through */
        case 2:
            retVal = msdGetAnyReg(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_VTU_DATA2_REG,&data2);
            if(retVal != MSD_OK)
            {
                return retVal;
            }
            /* pass through */
        case 1:
            retVal = msdGetAnyReg(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_VTU_DATA1_REG,&data1);
            if(retVal != MSD_OK)
            {
                return retVal;
            }
...
(
    ...
    IN      PEARL_MSD_STU_OPERATION    stuOp,
    INOUT   MSD_U8               *valid,
    INOUT    PEARL_MSD_STU_ENTRY        *entry
)
{
    MSD_STATUS       retVal;         /* Functions return value.      */
    ...
    if (stuOp == PEARL_LOAD_PURGE_STU_ENTRY)
    {
        if (*valid == 1)
        {
            /* set the Port State for all the ports */
            retVal = Pearl_stuSetSTUData(dev,entry);
            if(retVal != MSD_OK)
            {
                msdSemGive(dev->devNum, dev->vtuRegsSem);
                return retVal;
    ...
    if(stuOp == PEARL_GET_NEXT_STU_ENTRY)
    {
        ...
        /****************** get the valid bit *******************/
        retVal = msdGetAnyRegField(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_VTU_VID_REG,12,1,&data);
        if(retVal != MSD_OK)
        {
            msdSemGive(dev->devNum, dev->vtuRegsSem);
            return retVal;
        ...
        *valid = (MSD_U8)data;
        ...
        retVal = msdGetAnyReg(dev->devNum,  PEARL_GLOBAL1_DEV_ADDR,PEARL_QD_REG_STU_SID_REG,&data);
        if(retVal != MSD_OK)
        {
            msdSemGive(dev->devNum, dev->vtuRegsSem);
            return retVal;
        ...
        if (*valid)
        {
            ...
            retVal = Pearl_stuGetSTUData(dev,entry);
            if(retVal != MSD_OK)
            {
                msdSemGive(dev->devNum, dev->vtuRegsSem);
                return retVal;
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gstuFindSidEntry Called.\n"));
    ...
    *found = MSD_FALSE;
	...
	{
		MSD_DBG_INFO(("Pearl_gstuFindSidEntry Exit, MSD_NO_SUCH\n"));
		return MSD_OK;
	}
    ...
    *found = MSD_TRUE;
    ...
    MSD_DBG_INFO(("Pearl_gstuFindSidEntry Exit.\n"));
    return MSD_OK;
}
...
)
{
	...
	msdMemSet(&entry, 0, sizeof(PEARL_MSD_STU_ENTRY));
	entry.sid = 0x3f;
	...
	while (1 == temp)
	{
		status = Pearl_gstuGetEntryNext(dev, &entry);
