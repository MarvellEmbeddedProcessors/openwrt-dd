...
/*
 * Typedef: enum PEARL_MSD_PIRL_OPERATION
 *
 * Description: Defines the PIRL (Port Ingress Rate Limit) Operation type
 *
 * Fields:
 *      Pearl_PIRL_INIT_ALL_RESOURCE - Initialize all resources to the inital state
 *      Pearl_PIRL_INIT_RESOURCE     - Initialize selected resources to the inital state
 *      Pearl_PIRL_WRITE_RESOURCE    - Write to the selected resource/register
 *      Pearl_PIRL_READ_RESOURCE     - Read from the selected resource/register
 */
typedef enum
{
    Pearl_PIRL_INIT_ALL_RESOURCE      = 0x1,
    Pearl_PIRL_INIT_RESOURCE          = 0x2,
    Pearl_PIRL_WRITE_RESOURCE         = 0x3,
    Pearl_PIRL_READ_RESOURCE          = 0x4
} PEARL_MSD_PIRL_OPERATION;


/*
 *  typedef: struct PEARL_MSD_PIRL_OP_DATA
 *
...
typedef struct
{
    ...
    MSD_U32    irlReg;
    MSD_U32    irlData;
} PEARL_MSD_PIRL_OP_DATA;

/****************************************************************************/
/* STATS operation function declaration.                                    */
/****************************************************************************/
static MSD_STATUS Pearl_pirlOperationPerform
(
    IN    MSD_QD_DEV           *dev,
    IN    PEARL_MSD_PIRL_OPERATION   pirlOp,
    IN    MSD_U32              irlPort,
    IN    MSD_U32              irlRes,
    IN    MSD_U32              irlReg,
    INOUT PEARL_MSD_PIRL_OP_DATA     *opData
);
static MSD_STATUS Pearl_UpIntToConstant
(
	IN   MSD_U32 TimeUpInt,
	OUT  PEARL_MSD_PIRL_DATA    *pirlData
 );

static MSD_STATUS Pearl_ConstantToUpInt
(
...
)
{
    ...
    PEARL_MSD_PIRL_OPERATION    op;

    MSD_DBG_INFO(("Pearl_gpirlInitialize Called.\n"));

    op = Pearl_PIRL_INIT_ALL_RESOURCE;

    retVal = Pearl_pirlOperationPerform(dev, op, 0, 0, 0, NULL);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Pearl_pirlOperationPerform Pearl_PIRL_INIT_ALL_RESOURCE returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Pearl_gpirlInitialize Exit.\n"));
    return retVal;
}
...
)
{
    MSD_STATUS       retVal;    /* Functions return value */
    PEARL_MSD_PIRL_OPERATION    op;
    PEARL_MSD_PIRL_OP_DATA        opData;
	MSD_U8			port;

    MSD_DBG_INFO(("Pearl_gpirlInitResource Called.\n"));

    op = Pearl_PIRL_INIT_RESOURCE;
    ...
    retVal = Pearl_pirlOperationPerform(dev, op, port, irlRes, 0, &opData);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Pearl_pirlOperationPerform Pearl_PIRL_INIT_RESOURCE returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Pearl_gpirlInitResource Exit.\n"));
    return retVal;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gpirlCustomSetup Called.\n"));

	msdMemSet(&Setup,0,sizeof(PEARL_MSD_PIRL_CUSTOM_RATE_LIMIT));
	...
	if(customSetup->countMode == PEARL_MSD_PIRL_COUNT_FRAME)
		mode = 8.0;
    ...
    if (deltaMin == 1.0)
    {
        MSD_DBG_ERROR(("Failed (Pearl_gpirlCustomSetup tune failed).\n"));
        return MSD_FAIL;
    }
    ...
    MSD_DBG_INFO(("Pearl_gpirlCustomSetup Exit.\n"));
    return MSD_OK;	
#endif
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gpirlReadResource Called.\n"));

    op = Pearl_PIRL_READ_RESOURCE;
	port = MSD_LPORT_2_PORT(irlPort);
	if(port == MSD_INVALID_PORT)
	{
		MSD_DBG_ERROR(("Bad Port: %u.\n", (unsigned int)irlPort));
    ...
    retVal = Pearl_pirlOperationPerform(dev, op, 0xf, irlRes, 1, &opData);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Pearl_pirlOperationPerform Pearl_PIRL_READ_RESOURCE returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

	msdMemSet((void*)(pirlData), 0, sizeof(PEARL_MSD_PIRL_DATA));

	retVal = Pearl_UpIntToConstant((opData.irlData & 0x7), pirlData);
	if(retVal != MSD_OK)
	{
        MSD_DBG_ERROR(("Pearl_UpIntToConstantreturned: %s.\n", msdDisplayStatus(retVal)));
        return MSD_BAD_PARAM;
	}
    ...
    for(i=0; i<8; i++)
    {
        ...
        retVal = Pearl_pirlOperationPerform(dev, op, port, irlRes, i, &opData);
        if (retVal != MSD_OK)
        {
            MSD_DBG_ERROR(("Pearl_pirlOperationPerform Pearl_PIRL_READ_RESOURCE returned: %s.\n", msdDisplayStatus(retVal)));
            return retVal;
    ...
    /* reg6 data */
    pirlData->fcMode = ((data[6] >> 14) & 0x1)?PEARL_MSD_PIRL_FC_DEASSERT_CBS_LIMIT:PEARL_MSD_PIRL_FC_DEASSERT_EMPTY;
    pirlData->actionMode = ((data[6] >> 13) & 0x1)?Pearl_PIRL_ACTION_ACCEPT:Pearl_PIRL_ACTION_USE_LIMIT_ACTION;
    pirlData->ebsLimitAction = ((data[6] >> 12) & 0x1)?Pearl_EBS_LIMIT_ACTION_FC:Pearl_EBS_LIMIT_ACTION_DROP;
    ...
    MSD_DBG_INFO(("Pearl_gpirlReadResource Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gpirlWriteResource Called.\n"));

    op = Pearl_PIRL_WRITE_RESOURCE;
	port = MSD_LPORT_2_PORT(irlPort);
	if(port == MSD_INVALID_PORT)
	{
		MSD_DBG_ERROR(("Bad Port: %u.\n", (unsigned int)irlPort));
	...
	retVal = Pearl_ConstantToUpInt(pirlData, &opData.irlData);
	if(retVal != MSD_OK)
	{
    ...
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Pearl_pirlOperationPerform Pearl_PIRL_WRITE_RESOURCE returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    MSD_DBG_INFO(("Pearl_gpirlWriteResource Exit.\n"));
    return MSD_OK;  
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gpirlGetResReg Called.\n"));
	...
	retVal = Pearl_pirlOperationPerform(dev, op, port, irlRes, regOffset, &opData);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Pearl_pirlOperationPerform Pearl_PIRL_READ_RESOURCE returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
	...
	*data = (MSD_U16)opData.irlData;
    ...
    MSD_DBG_INFO(("Pearl_gpirlGetResReg Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gpirlSetResReg Called.\n"));
    ...
    MSD_DBG_INFO(("Pearl_gpirlSetResReg Exit.\n"));
    return MSD_OK;  
}
...
)
{
    ...
    while(data == 1)
    {
        retVal = msdGetAnyRegField(dev->devNum,  PEARL_GLOBAL2_DEV_ADDR,PEARL_QD_REG_IGR_RATE_COMMAND,15,1,&data);
        if(retVal != MSD_OK)
        {
    ...
    switch (pirlOp)
    {
        case Pearl_PIRL_INIT_ALL_RESOURCE:
            data = (1 << 15) | (Pearl_PIRL_INIT_ALL_RESOURCE << 12);
            retVal = msdSetAnyReg(dev->devNum,  PEARL_GLOBAL2_DEV_ADDR,PEARL_QD_REG_IGR_RATE_COMMAND,data);
            if(retVal != MSD_OK)
            {
                msdSemGive(dev->devNum, dev->pirlRegsSem);
                return retVal;
            ...
            break;
        case Pearl_PIRL_INIT_RESOURCE:
            data = (MSD_U16)((1 << 15) | (Pearl_PIRL_INIT_RESOURCE << 12) | (irlPort&0xf)<<8 | 
                    ((irlRes&0x7)<< 5));
            retVal = msdSetAnyReg(dev->devNum,  PEARL_GLOBAL2_DEV_ADDR,PEARL_QD_REG_IGR_RATE_COMMAND,data);
            if(retVal != MSD_OK)
            {
        ...
        case Pearl_PIRL_WRITE_RESOURCE:
            data = (MSD_U16)opData->irlData;
            retVal = msdSetAnyReg(dev->devNum,  PEARL_GLOBAL2_DEV_ADDR,PEARL_QD_REG_IGR_RATE_DATA,data);
            if(retVal != MSD_OK)
            {
                msdSemGive(dev->devNum, dev->pirlRegsSem);
                return retVal;
            ...
            data = (MSD_U16)((1 << 15) | (Pearl_PIRL_WRITE_RESOURCE << 12) | (irlPort&0xf)<<8 | 
		...
		case Pearl_PIRL_READ_RESOURCE:
			data = (MSD_U16)((1 << 15) | (Pearl_PIRL_READ_RESOURCE << 12) | (irlPort & 0xf) << 8 |
                    ((irlRes&0x7) << 5) | (irlReg & 0xF));
            retVal = msdSetAnyReg(dev->devNum,  PEARL_GLOBAL2_DEV_ADDR,PEARL_QD_REG_IGR_RATE_COMMAND,data);
            if(retVal != MSD_OK)
            {
            ...
            retVal = msdGetAnyReg(dev->devNum,  PEARL_GLOBAL2_DEV_ADDR,PEARL_QD_REG_IGR_RATE_DATA,&data);
