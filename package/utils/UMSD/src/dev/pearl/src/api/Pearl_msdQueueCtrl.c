...
)
{
	...
	MSD_DBG_INFO(("Pearl_gprtSetPortSched Called.\n"));
	...
	switch (mode)
	{
	case PEARL_MSD_PORT_SCHED_WRR_PRI3_2_1_0:
		data = 0;
		break;
	case PEARL_MSD_PORT_SCHED_WRR_PRI2_1_0:
		data = 1;
		break;
	case PEARL_MSD_PORT_SCHED_WRR_PRI1_0:
		data = 2;
		break;
	case PEARL_MSD_PORT_SCHED_STRICT_PRI_ALL:
		data = 3;
		break;
	default:
	...
	retVal = msdSetAnyRegField(dev->devNum, phyAddr, PEARL_QD_REG_EGRESS_RATE_CTRL2, 12, 2, data);
	if (retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Write port PEARL_QD_REG_EGRESS_RATE_CTRL2 register returned: %s.\n", msdDisplayStatus(retVal)));
		return retVal;
	}

	MSD_DBG_INFO(("Pearl_gprtSetPortSched Exit.\n"));
...
)
{
	...
	MSD_DBG_INFO(("Pearl_gprtGetPortSched Called.\n"));
	...
	retVal = msdGetAnyRegField(dev->devNum, phyAddr, PEARL_QD_REG_EGRESS_RATE_CTRL2, 12, 2, &data);
	if (retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Read port PEARL_QD_REG_EGRESS_RATE_CTRL2 register returned: %s.\n", msdDisplayStatus(retVal)));
		return retVal;
	}
	...
	{
	case 0x0:
		*mode = PEARL_MSD_PORT_SCHED_WRR_PRI3_2_1_0;
		break;
	case 0x1:
		*mode = PEARL_MSD_PORT_SCHED_WRR_PRI2_1_0;
		break;
	case 0x2:
		*mode = PEARL_MSD_PORT_SCHED_WRR_PRI1_0;
		break;
	case 0x3:
		*mode = PEARL_MSD_PORT_SCHED_STRICT_PRI_ALL;
		break;
	default:
		MSD_DBG_ERROR(("Bad Mode: %u.\n", (unsigned int)data));
		return MSD_BAD_PARAM;
	...
	MSD_DBG_INFO(("Pearl_gprtGetPortSched Exit.\n"));
	return retVal;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gsysSetQoSWeight Called.\n"));
    ...
    for (i = 0; i<len; i++)
    {
        ...
        while (data == 1)
        {
            retVal = msdGetAnyRegField(dev->devNum, PEARL_GLOBAL2_DEV_ADDR,
                PEARL_QD_REG_QOS_WEIGHT, 15, 1, &data);
            if (retVal != MSD_OK)
            {
        ...
        retVal = msdSetAnyReg(dev->devNum, PEARL_GLOBAL2_DEV_ADDR, PEARL_QD_REG_QOS_WEIGHT, data);
        if (retVal != MSD_OK)
        {
            MSD_DBG_INFO(("Failed.\n"));
            msdSemGive(dev->devNum, dev->tblRegsSem);
    ...
    MSD_DBG_INFO(("Pearl_gsysSetQoSWeight Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Pearl_gsysGetQoSWeight Called.\n"));
    ...
    retVal = msdGetAnyReg(dev->devNum, PEARL_GLOBAL2_DEV_ADDR, PEARL_QD_REG_QOS_WEIGHT, &data);
    if (retVal != MSD_OK)
    {
        MSD_DBG_INFO(("Failed.\n"));
        msdSemGive(dev->devNum, dev->tblRegsSem);
