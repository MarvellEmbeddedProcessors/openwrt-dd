...
{
	PEARL_MSD_TCAM_DATA tcam;
	MSD_U8 i=0;

	if (NULL == tcamData)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_DATA in Pearl_gtcamLoadEntryIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}

	if (tcamData->dpvMode == 1 || tcamData->dpvMode == 2)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_DATA in Pearl_gtcamLoadEntryIntf is not support dpvMode = %d. \n",tcamData->dpvMode));
	...
	return Pearl_gtcamLoadEntry(dev, tcamPointer, &tcam);
}
/*******************************************************************************
...
)
{
	...
	retVal=Pearl_gtcamReadTCAMData(dev, tcamPointer, &tcam);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == tcamData)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_DATA in Pearl_gtcamReadTCAMDataIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
...
)
{
	...
	retVal=Pearl_gtcamGetNextTCAMData(dev, tcamPointer, &tcam);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == tcamData)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_DATA in Pearl_gtcamGetNextTCAMDataIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
	msdMemSet((void*)(tcamData), 0, sizeof(MSD_TCAM_DATA));
...
)
{
    ...
    msdMemSet((void*)(&tcam), 0, sizeof(PEARL_MSD_TCAM_DATA));
	retVal = Pearl_gtcamFindEntry(dev, tcamPointer, &tcam, found);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == tcamData)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_DATA in Pearl_gtcamFindEntryIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
	msdMemSet((void*)(tcamData), 0, sizeof(MSD_TCAM_DATA));
...
{
    return Pearl_gtcamSetMode(dev, portNum, (PEARL_MSD_TCAM_MODE)mode);
}
/*******************************************************************************
* Pearl_gtcamGetModeIntf
*
* DESCRIPTION:
*       Set the TCAM Mode, The following mode is supported:
*               TCAM_MODE_DISABLE, TCAM_MODE_ENABLE_48, TCAM_MODE_ENABLE_96
...
{
    MSD_STATUS retVal;
    PEARL_MSD_TCAM_MODE temp;

    if (mode == NULL)
    {
        MSD_DBG_ERROR(("Input param MSD_TCAM_MODE in Pearl_gtcamGetModeIntf is NULL. \n"));
        return MSD_BAD_PARAM;
    }

    retVal = Pearl_gtcamGetMode(dev, portNum, &temp);
...
{
    PEARL_MSD_TCAM_ENT_KEY tmpKeyMask;
    PEARL_MSD_TCAM_ENT_KEY tmpKey;
    PEARL_MSD_TCAM_ENT_ACT tmpAction;
    ...
    return Pearl_gtcamAdvConfig(dev, pktType, tcamPointer, tcamPointer2, &tmpKeyMask, &tmpKey, maskPtr, patternPtr, &tmpAction, entry2Used);
}