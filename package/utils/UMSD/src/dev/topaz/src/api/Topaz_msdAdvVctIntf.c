...
{
	TOPAZ_MSD_ADV_VCT_MODE  vctmode;
    TOPAZ_MSD_ADV_CABLE_STATUS cable; 
    MSD_U8 i = 0, j = 0;
	MSD_STATUS  retVal;
	...
	retVal=Topaz_gvctGetAdvCableDiag(dev, port, vctmode, &cable);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == cableStatus)
	{
		MSD_DBG_ERROR(("Input param MSD_ADV_CABLE_STATUS in Topaz_gvctGetAdvCableDiagIntf is NULL. \n"));
...
{
	TOPAZ_MSD_ADV_EXTENDED_STATUS extend;
    MSD_STATUS retVal; 
    MSD_U8 i = 0;

	retVal=Topaz_gvctGetAdvExtendedStatus(dev, port, &extend);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == extendedStatus)
	{
		MSD_DBG_ERROR(("Input param MSD_ADV_EXTENDED_STATUS in Topaz_gvctGetAdvExtendedStatusIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
