...
#define TOPAZ_MSD_STATS_NO_OP               0x0
#define TOPAZ_MSD_STATS_FLUSH_ALL           0x1
#define TOPAZ_MSD_STATS_FLUSH_PORT          0x2
#define TOPAZ_MSD_STATS_READ_COUNTER        0x4
#define TOPAZ_MSD_STATS_CAPTURE_PORT        0x5
#define TOPAZ_MSD_STATS_CAPTURE_PORT_CLEAR  0x6
...
typedef enum 
{
    Topaz_STATS_FLUSH_ALL,            /* Flush all counters for all ports */
    Topaz_STATS_FLUSH_PORT,           /* Flush all counters for a port */
    Topaz_STATS_READ_COUNTER,         /* Read a specific counter from a port */
    Topaz_STATS_READ_REALTIME_COUNTER,  /* Read a realtime counter from a port */
    Topaz_STATS_READ_ALL,              /* Read all counters from a port */
    Topaz_STATS_READ_COUNTER_CLEAR,   /* For RMU page2,Read a specific counter from a port and clear*/
    Topaz_STATS_READ_ALL_CLEAR        /* For RMU page2,Read all counters from a port and clear*/
} TOPAZ_MSD_STATS_OPERATION;

/****************************************************************************/
/* STATS operation function declaration.                                    */
/****************************************************************************/
static MSD_STATUS Topaz_statsOperationPerform
(
    IN   MSD_QD_DEV            *dev,
    IN   TOPAZ_MSD_STATS_OPERATION   statsOp,
    IN   MSD_U8                port,
    IN   TOPAZ_MSD_STATS_COUNTERS    counter,
    OUT  MSD_VOID              *statsData
);

static MSD_STATUS Topaz_statsReadCounter
(
    IN   MSD_QD_DEV      *dev,
    IN   MSD_U16         port,
    IN   MSD_U32         counter,
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gstatsFlushAll Called.\n"));

    retVal = Topaz_statsOperationPerform(dev,Topaz_STATS_FLUSH_ALL,0,0,NULL);
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Topaz_statsOperationPerform FLUSH_ALL returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Topaz_gstatsFlushAll Exit.\n"));
    return MSD_OK;
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gstatsFlushPort Called.\n"));
    ...
    retVal = Topaz_statsOperationPerform(dev,Topaz_STATS_FLUSH_PORT,hwPort,0,NULL);
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Topaz_statsOperationPerform FLUSH_PORT returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Topaz_gstatsFlushPort Exit.\n"));
    return MSD_OK;
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gstatsGetPortCounter Called.\n"));
    ...
    if (counter > TOPAZ_MSD_TYPE_BANK + 0x1F || counter < 0 || (counter > 0x1f && counter < TOPAZ_MSD_TYPE_BANK))
    {
        MSD_DBG_ERROR(("Bad counter: %d.\n", (int)counter));
        return MSD_BAD_PARAM;
    }

    retVal = Topaz_statsOperationPerform(dev,Topaz_STATS_READ_COUNTER,hwPort,counter,(MSD_VOID*)statsData);
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Topaz_statsOperationPerform READ_COUNTER returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Topaz_gstatsGetPortCounter Exit.\n"));
    return MSD_OK;
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gstatsGetPortAllCounters Called.\n"));
    ...
    retVal = Topaz_statsOperationPerform(dev,Topaz_STATS_READ_ALL,hwPort,0,(MSD_VOID*)statsCounterSet);
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Topaz_statsOperationPerform READ_ALL returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Topaz_gstatsGetPortAllCounters Exit.\n"));
    return MSD_OK;
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gstatsGetHistogramMode Called.\n"));
	...
	retVal = msdGetAnyRegField(dev->devNum,  TOPAZ_GLOBAL1_DEV_ADDR,TOPAZ_QD_REG_GLOBAL_CONTROL2,6,2,&data);	
    if(retVal != MSD_OK)
    {
	...
	{
	case 0x1:
		*mode = TOPAZ_MSD_COUNT_RX_ONLY;
		break;
	case 0x2:
		*mode = TOPAZ_MSD_COUNT_TX_ONLY;
		break;
	case 0x3:
		*mode = TOPAZ_MSD_COUNT_RX_TX;
		break;
	default:
    ...
    MSD_DBG_INFO(("Topaz_gstatsGetHistogramMode Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gstatsSetHistogramMode Called.\n"));

    switch (mode)
    {
	    case TOPAZ_MSD_COUNT_RX_ONLY:
	    case TOPAZ_MSD_COUNT_TX_ONLY:
	    case TOPAZ_MSD_COUNT_RX_TX:
            break;
        default:
	...
	retVal = msdSetAnyRegField(dev->devNum,  TOPAZ_GLOBAL1_DEV_ADDR,TOPAZ_QD_REG_GLOBAL_CONTROL2,6,2,data);
    if(retVal != MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Topaz_gstatsSetHistogramMode Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    while(data == 1)
    {
        retVal = msdGetAnyRegField(dev->devNum,  TOPAZ_GLOBAL1_DEV_ADDR,TOPAZ_QD_REG_STATS_OPERATION,15,1,&data);
        if(retVal != MSD_OK)
        {
    ...
    switch (statsOp)
    {
        case Topaz_STATS_FLUSH_ALL:
            data |= (1 << 15) | (TOPAZ_MSD_STATS_FLUSH_ALL << 12);
            retVal = msdSetAnyReg(dev->devNum,  TOPAZ_GLOBAL1_DEV_ADDR,TOPAZ_QD_REG_STATS_OPERATION,data);
			
			data |= (1 << 15) | (TOPAZ_MSD_STATS_FLUSH_ALL << 12) | (1<<10);
			retVal = msdSetAnyReg(dev->devNum,  TOPAZ_GLOBAL1_DEV_ADDR,TOPAZ_QD_REG_STATS_OPERATION,data);

            msdSemGive(dev->devNum, dev->statsRegsSem);
            return retVal;

        case Topaz_STATS_FLUSH_PORT:
            data |= (1 << 15) | (TOPAZ_MSD_STATS_FLUSH_PORT << 12) | portNum ;
            retVal = msdSetAnyReg(dev->devNum,  TOPAZ_GLOBAL1_DEV_ADDR,TOPAZ_QD_REG_STATS_OPERATION,data);
			data |= (1 << 15) | (TOPAZ_MSD_STATS_FLUSH_PORT << 12) | portNum | (1<<10);
			retVal = msdSetAnyReg(dev->devNum,  TOPAZ_GLOBAL1_DEV_ADDR,TOPAZ_QD_REG_STATS_OPERATION,data);
			
            msdSemGive(dev->devNum, dev->statsRegsSem);
            return retVal;

        case Topaz_STATS_READ_COUNTER:
            retVal = Topaz_statsReadCounter(dev, portNum, (MSD_U32)counter, (MSD_U32*)statsData);
            if(retVal != MSD_OK)
            {
                msdSemGive(dev->devNum, dev->statsRegsSem);
                return retVal;
        ...
        case Topaz_STATS_READ_ALL:
			{
				int bankSize = 2;
				for(bank=0; bank<bankSize; bank++)
				{
					lastCounter = (bank==0)?(MSD_U32)Topaz_STATS_Late : (MSD_U32)Topaz_STATS_OutMGMT;
					startCounter = (bank==0)?(MSD_U32)Topaz_STATS_InGoodOctetsLo : (MSD_U32)Topaz_STATS_InDiscards;

					if(bank==1)
						statsData = (unsigned int *)statsData + Topaz_STATS_Late +1;
					for(tmpCounter=startCounter; tmpCounter<=lastCounter; tmpCounter++)
					{
						retVal = Topaz_statsReadCounter(dev, portNum, tmpCounter,((MSD_U32*)statsData+tmpCounter-startCounter));
						if(retVal != MSD_OK)
						{
							msdSemGive(dev->devNum, dev->statsRegsSem);
							return retVal;
...
)
{
    ...
    data = (MSD_U16)((1 << 15) | (TOPAZ_MSD_STATS_READ_COUNTER << 12) | port | (counter&0x1f) );
	if (counter & TOPAZ_MSD_TYPE_BANK)
	{
		data |= (1<<10);
	}
    ...
    retVal = msdGetAnyReg(dev->devNum,  TOPAZ_GLOBAL1_DEV_ADDR,TOPAZ_QD_REG_STATS_COUNTER3_2,&counter3_2);
    if(retVal != MSD_OK)
    {
        return retVal;
    }

    retVal = msdGetAnyReg(dev->devNum,  TOPAZ_GLOBAL1_DEV_ADDR,TOPAZ_QD_REG_STATS_COUNTER1_0,&counter1_0);
    if(retVal != MSD_OK)
    {
        return retVal;
    }
...
)
{
	...
	msdMemSet(&statsCounter, 0, sizeof(TOPAZ_MSD_STATS_COUNTER_SET));

	if ((status = Topaz_gstatsGetPortAllCounters(dev, portNum, &statsCounter)) != MSD_OK)
	{
		return status;
	}
	/* Bank 0 */
