...
)
{
	...
	MSD_DBG_INFO(("Topaz_msdRmuGetID Called.\n"));
    ...
    *id = ((*(rspEthPacketPtr + 24 - delta) & 0xff) << 8) | (*(rspEthPacketPtr + 25 - delta) & 0xff);
	...
	MSD_DBG_INFO(("Topaz_msdRmuGetID Exit.\n"));
	return MSD_OK;
}
...
)
{
	...
	MSD_DBG_INFO(("Topaz_msdRmuAtuDump Called.\n"));
	...
	*startAddr = next_cont_code;
	...
	MSD_DBG_INFO(("Topaz_msdRmuAtuDump Exit.\n"));
...
)
{
	...
	MSD_DBG_INFO(("Topaz_msdRmuMib2Dump Called.\n"));
	...
	MSD_DBG_INFO(("Topaz_rmuMib2Dump Exit.\n"));
...
)
{
	...
	MSD_DBG_INFO(("Topaz_msdRmuReadRegister Called.\n"));
	...
	MSD_DBG_INFO(("Topaz_msdRmuReadRegister Exit.\n"));
...
)
{
	...
	MSD_DBG_INFO(("Topaz_msdRmuWriteRegister Called.\n"));
	...
	MSD_DBG_INFO(("Topaz_msdRmuWriteRegister Exit.\n"));
...
)
{
	...
	MSD_DBG_INFO(("Topaz_msdRmuMultiRegAccess Called.\n"));
	...
	MSD_DBG_INFO(("Topaz_msdRmuMultiRegAccess Exit.\n"));
...
)
{
	...
	MSD_DBG_INFO(("Topaz_msdRmuTCAMDump Called.\n"));
	...
	*startAddr = dump_TCAMPacket(delta, rspEthPacketPtr, rsp_pktlen, numOfEntry, tcamEntry);
	...
	MSD_DBG_INFO(("Topaz_rmuTCAMDump Exit.\n"));
	return MSD_OK;
}
...
)
{
	...
	MSD_DBG_INFO(("Topaz_msdRmuTCAMWrite Called.\n"));

	if (tcamPointer >= TOPAZ_MAX_IGR_TCAM_ENTRY || tcamEntry == NULL)
	{
	...
	MSD_DBG_INFO(("Topaz_msdRmuTCAMWrite Exit.\n"));
	return MSD_OK;
}
...
)
{
	...
	MSD_DBG_INFO(("Topaz_msdRmuTCAMRead Called.\n"));
	...
	MSD_DBG_INFO(("Topaz_msdRmuTCAMRead Exit.\n"));
	return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_msdRmuRegDump Called.\n"));
    ...
    if ((retVal = Topaz_msdRmuMultiRegAccess(dev, &pRegCmd, nCmd)) != MSD_OK)
    {
        MSD_DBG_ERROR(("Topaz_msdRmuMultiRegAccess returned fail.\n"));
        return retVal;
    }
    ...
    MSD_DBG_INFO(("Topaz_msdRmuRegDump Exit.\n"));
    return MSD_OK;
}
...
)
{
	...
	while (flag) {
		...
		((*atuEntry) + atu_cnt)->trunkMember = ((*ptr) >> 3) & 0x01;
...
)
{
    ...
    temp |= ((*ptr) & 0xff) << 8; ptr++;
    temp |= ((*ptr) & 0xff) << 0; ptr++;
    statsCounterSet->reserved_1C = temp;
    temp = 0;
    temp |= ((*ptr) & 0xff) << 24; ptr++;
    temp |= ((*ptr) & 0xff) << 16; ptr++;
    temp |= ((*ptr) & 0xff) << 8; ptr++;
...
)
{
	...
	while (flag) {
		...
		/*if (idx == 0)*/
		{
			...
			((*tcamEntry) + tcam_cnt)->spvMask = (MSD_BF_GET(tcam_temp, 8, 7)) & 0x7f;
			((*tcamEntry) + tcam_cnt)->spv = (MSD_BF_GET(tcam_temp, 0, 7)) & 0x7f;
			...
			((*tcamEntry) + tcam_cnt)->qpriData = MSD_BF_GET(tcam_temp, 4, 2);
			...
			((*tcamEntry) + tcam_cnt)->dpvData = (MSD_BOOL)MSD_BF_GET(tcam_temp, 0, 7);
...
)
{
	...
	if (MSD_RMU_TCAM_OPCODE_WRITE == opCode)
	{
		...
		temp = 0;
		MSD_BF_SET(temp, (entry->spvMask & 0x7f), 8, 7);
		MSD_BF_SET(temp, (entry->spv & 0x7f), 0, 7);
		msdMemCpy(pktPtr, &temp, sizeof(temp)); pktPtr += 1;
		...
		MSD_BF_SET(temp, entry->qpriData, 4, 2);
		...
		MSD_BF_SET(temp, (MSD_U16)(entry->dpvData), 0, 7);
		msdMemCpy(pktPtr, &temp, sizeof(temp)); pktPtr += 1;
