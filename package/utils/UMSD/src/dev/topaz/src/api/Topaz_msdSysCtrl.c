...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysSetTrunkMaskTable Called.\n"));

	if (maskNum >= TOPAZ_MAX_LAG_MASK_NUM)
	{
		MSD_DBG_ERROR(("MSD_BAD_PARAM (maskNum: %u). It should be within [0, %d]\n", (unsigned int)maskNum, (TOPAZ_MAX_LAG_MASK_NUM - 1)));
    ...
    do
    {
		retVal = msdGetAnyReg(dev->devNum, TOPAZ_GLOBAL2_DEV_ADDR,TOPAZ_QD_REG_TRUNK_MASK_TBL,&data);
        if(retVal != MSD_OK)
        {
	...
	retVal = msdSetAnyReg(dev->devNum, TOPAZ_GLOBAL2_DEV_ADDR,TOPAZ_QD_REG_TRUNK_MASK_TBL,data);
    ...
    MSD_DBG_INFO(("Topaz_gsysSetTrunkMaskTable Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysGetTrunkMaskTable Called.\n"));
    ...
    *trunkMask = MSD_PORTVEC_2_LPORTVEC(data & mask);
    ...
    MSD_DBG_INFO(("Topaz_gsysGetTrunkMaskTable Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysSetHashTrunk Called.\n"));

    MSD_BOOL_2_BIT(en,data);

    /* Set related bit */
	retVal = msdSetAnyRegField(dev->devNum, TOPAZ_GLOBAL2_DEV_ADDR,TOPAZ_QD_REG_TRUNK_MASK_TBL,11,1,data);
    if(retVal != MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Topaz_gsysSetHashTrunk Exit.\n"));
    return MSD_OK;
}
...
)
{
    MSD_STATUS       retVal;         /* Functions return value.      */
    MSD_U16          data;           /* The register's read data.    */
    MSD_DBG_INFO(("Topaz_gsysGetHashTrunk Called.\n"));

    /* Get related bit */
    retVal = msdGetAnyRegField(dev->devNum, TOPAZ_GLOBAL2_DEV_ADDR,TOPAZ_QD_REG_TRUNK_MASK_TBL,11,1,&data);
    if(retVal != MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Topaz_gsysGetHashTrunk Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysSetTrunkRouting Called.\n"));
	...
	/* Check if trunkRoute is our of range [0,0x7f] */
    mask = (1 << dev->numOfPorts) - 1;
    if(trunkRoute > mask)
    {
    ...
    do
    {
		retVal = msdGetAnyReg(dev->devNum, TOPAZ_GLOBAL2_DEV_ADDR,TOPAZ_QD_REG_TRUNK_ROUTING,&data);
        if(retVal != MSD_OK)
        {
	...
	retVal = msdSetAnyReg(dev->devNum, TOPAZ_GLOBAL2_DEV_ADDR,TOPAZ_QD_REG_TRUNK_ROUTING,data);
    ...
    MSD_DBG_INFO(("Topaz_gsysSetTrunkRouting Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysGetTrunkRouting Called.\n"));
    ...
    *trunkRoute = MSD_PORTVEC_2_LPORTVEC(data & mask);
    ...
    MSD_DBG_INFO(("Topaz_gsysGetTrunkRouting Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysSetFloodBC Called.\n"));

    MSD_BOOL_2_BIT(en,data);

    /* Set related bit */
    retVal = msdSetAnyRegField(dev->devNum, TOPAZ_GLOBAL2_DEV_ADDR, TOPAZ_QD_REG_MANAGEMENT, 12, 1, data);
    if(retVal != MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Topaz_gsysSetFloodBC Exit.\n"));
    return MSD_OK;
}
...
)
{
    MSD_STATUS       retVal;         /* Functions return value.      */
    MSD_U16          data;           /* The register's read data.    */
    MSD_DBG_INFO(("Topaz_gsysGetFloodBC Called.\n"));

    /* Get related bit */
    retVal = msdGetAnyRegField(dev->devNum,  TOPAZ_GLOBAL2_DEV_ADDR, TOPAZ_QD_REG_MANAGEMENT,12,1,&data);
    if(retVal != MSD_OK)
    {
    ...
    MSD_BIT_2_BOOL(data,*en);
    MSD_DBG_INFO(("Topaz_gsysGetFloodBC Exit.\n"));
    return MSD_OK;
}
...
)
{
	...
	MSD_U16            tmpData;
    int count=0x10;
    MSD_DBG_INFO(("Topaz_gsysSetMonitorMgmtCtrl Called.\n"));
    ...
    while(tmpData ==1)
    {
		retVal = msdGetAnyRegField(dev->devNum, TOPAZ_GLOBAL1_DEV_ADDR, TOPAZ_QD_REG_MONITOR_MGMT_CTRL,15,1,&tmpData);
		if(retVal != MSD_OK)
		{
		...
		if((count--)==0) 
		{
			...
			msdSemGive(dev->devNum, dev->tblRegsSem);
			MSD_DBG_INFO(("Failed (Time out for wait busy bit).\n"));
			return MSD_FAIL;
		}
    }

	tmpData =  (MSD_U16)((1 << 15) | (point << 8) | data);
	retVal = msdSetAnyReg(dev->devNum,  TOPAZ_GLOBAL1_DEV_ADDR, TOPAZ_QD_REG_MONITOR_MGMT_CTRL, tmpData);
    ...
    MSD_DBG_INFO(("Topaz_gsysSetMonitorMgmtCtrl Exit.\n"));
	return retVal;
}
...
)
{
	...
	MSD_U16	tmpData;
	int count=0x10;
    MSD_DBG_INFO(("Topaz_gsysGetMonitorMgmtCtrl Called.\n"));
	...
	do {
        retVal = msdGetAnyReg(dev->devNum,  TOPAZ_GLOBAL1_DEV_ADDR, TOPAZ_QD_REG_MONITOR_MGMT_CTRL, &tmpData);
        if(retVal != MSD_OK)
		{
			MSD_DBG_ERROR(("Read G1 monitor&mgmt register busy bit returned: %s.\n", msdDisplayStatus(retVal)));
			msdSemGive(dev->devNum, dev->tblRegsSem);
	...
	*data = (MSD_U8)(tmpData&0xff);
    ...
    MSD_DBG_INFO(("Topaz_gsysGetMonitorMgmtCtrl Exit.\n"));
    return retVal;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysSetCPUDest Called.\n"));

    /* translate LPORT to hardware port */
    hwPort = MSD_LPORT_2_PORT(port);

    /* translate LPORT to hardware port */
    if(port == 0x1F || port == 0x06)
        hwPort = (MSD_U8)port;
    else
    {
	...
	retVal = Topaz_gsysGetMonitorMgmtCtrl(dev,0x30,&data);
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Topaz_gsysGetMonitorMgmtCtrl returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

	data = ((data & 0xE0) | hwPort);
	retVal = Topaz_gsysSetMonitorMgmtCtrl(dev,0x30, (MSD_U8)data);
	if(retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Topaz_gsysSetMonitorMgmtCtrl returned: %s.\n", msdDisplayStatus(retVal)));
		return retVal;
	}

	
    MSD_DBG_INFO(("Topaz_gsysSetCPUDest Exit.\n"));
	return retVal;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysGetCPUDest Called.\n"));

    retVal = Topaz_gsysGetMonitorMgmtCtrl(dev,0x30, &datac);
    if(retVal != MSD_OK)
	{
	    MSD_DBG_ERROR(("Topaz_gsysGetMonitorMgmtCtrl returned: %s.\n", msdDisplayStatus(retVal)));
	    return retVal;
    ...
    MSD_DBG_INFO(("Topaz_gsysGetCPUDest Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysSetIngressMonitorDest Called.\n"));
	...
	retVal = Topaz_gsysSetMonitorMgmtCtrl(dev, 0x20, (MSD_U8)hwPort);
	if(retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Topaz_gsysSetMonitorMgmtCtrl returned: %s.\n", msdDisplayStatus(retVal)));
		return retVal;
    ...
    MSD_DBG_INFO(("Topaz_gsysSetIngressMonitorDest Exit.\n"));
	return retVal;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysGetIngressMonitorDest Called.\n"));

	retVal = Topaz_gsysGetMonitorMgmtCtrl(dev, 0x20, &datac);
    if(retVal != MSD_OK)
	{
	    MSD_DBG_ERROR(("Topaz_gsysGetMonitorMgmtCtrl returned: %s.\n", msdDisplayStatus(retVal)));
	    return retVal;
    ...
    MSD_DBG_INFO(("Topaz_gsysGetIngressMonitorDest Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysSetEgressMonitorDest Called.\n"));
	...
	retVal = Topaz_gsysSetMonitorMgmtCtrl(dev, 0x21, (MSD_U8)hwPort);
    if(retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Topaz_gsysSetMonitorMgmtCtrl returned: %s.\n", msdDisplayStatus(retVal)));
	    return retVal;
    ...
    MSD_DBG_INFO(("Topaz_gsysSetEgressMonitorDest Exit.\n"));
    return retVal;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysGetEgressMonitorDest Called.\n"));
		
	retVal = Topaz_gsysGetMonitorMgmtCtrl(dev, 0x21, &datac);
    if(retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Topaz_gsysGetMonitorMgmtCtrl returned: %s.\n", msdDisplayStatus(retVal)));
	    return retVal;
    ...
    MSD_DBG_INFO(("Topaz_gsysGetEgressMonitorDest Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysSetRsvd2CpuEnables Called.\n"));

	retVal = Topaz_gsysSetMonitorMgmtCtrl(dev, 0x0, (MSD_U8)(enBits & 0xff));
	if(retVal != MSD_OK)
	{
	    MSD_DBG_ERROR(("Topaz_gsysSetMonitorMgmtCtrl for point 0 returned: %s.\n", msdDisplayStatus(retVal)));
		return retVal;
	}
	retVal = Topaz_gsysSetMonitorMgmtCtrl(dev, 0x1, (MSD_U8)((enBits >> 8) & 0xff));
	if(retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Topaz_gsysSetMonitorMgmtCtrl for point 1 returned: %s.\n", msdDisplayStatus(retVal)));
		return retVal;
	}
	    
	MSD_DBG_INFO(("Topaz_gsysSetRsvd2CpuEnables Exit.\n"));
    return retVal;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysGetRsvd2CpuEnables Called.\n"));

	retVal = Topaz_gsysGetMonitorMgmtCtrl(dev, 0x0, &data);
	if(retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Topaz_gsysGetMonitorMgmtCtrl for point 0 returned: %s.\n", msdDisplayStatus(retVal)));
		return retVal;
	}
	*enBits = data;
	retVal = Topaz_gsysGetMonitorMgmtCtrl(dev, 0x1, &data);
	if(retVal != MSD_OK)
	{
		MSD_DBG_ERROR(("Topaz_gsysGetMonitorMgmtCtrl for point 1 returned: %s.\n", msdDisplayStatus(retVal)));
		return retVal;
	}
	dataw=data;
	dataw <<=8;
	*enBits |= (dataw&0xff00);
	...
	MSD_DBG_INFO(("Topaz_gsysGetRsvd2CpuEnables Exit.\n"));
	return retVal;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysSetMGMTPri Called.\n"));
	...
	retVal = Topaz_gsysSetMonitorMgmtCtrl(dev, 0x30, data);
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Topaz_gsysSetMonitorMgmtCtrl returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    ...
    MSD_DBG_INFO(("Topaz_gsysSetMGMTPri Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gsysGetMGMTPri Called.\n"));
	...
	*pri = ((data & 0xE0) >> 5);
    ...
    MSD_DBG_INFO(("Topaz_gsysGetMGMTPri Exit.\n"));
    return MSD_OK;
}
...
)
{
	...
	char desc[32][48] =
	{
		...
		{ "VTU/STU Data Register Ports 0 to 3(0x7)" },
		{ "VTU/STU Data Register Ports 4 to 6(0x8)" },
		{ "VTU/STU Data Register For VTU Ops(0x9)" },
		...
		{ "IP-PRI Mapping Register 0(0x10)" },
		{ "IP-PRI Mapping Register 1(0x11)" },
		{ "IP-PRI Mapping Register 2(0x12)" },
		{ "IP-PRI Mapping Register 3(0x13)" },
		{ "IP-PRI Mapping Register 4(0x14)" },
		{ "IP-PRI Mapping Register 5(0x15)" },
		{ "IP-PRI Mapping Register 6(0x16)" },
		{ "IP-PRI Mapping Register 7(0x17)" },
		{ "IEEE-PRI Register(0x18)" },
		{ "IP Mapping Table(0x19)" },
	...
	MSD_DBG_INFO(("Topaz_gsysGlobal1RegDump Called.\n"));

	MSG(("\n------------------------------------------------------\n"));
	for (i = 0; i < 32; i++)
	{
		retVal = msdGetAnyReg(dev->devNum, TOPAZ_GLOBAL1_DEV_ADDR, i, &data);
		if (retVal != MSD_OK)
		{
	...
	MSD_DBG_INFO(("Topaz_gsysGlobal1RegDump Exit.\n"));
	return MSD_OK;
}
...
)
{
	...
	char desc[32][48] =
	{
		...
		{ "EEPROM Command(0x14)" },
		{ "EEPROM Addr(0x15)" },
		{ "AVB Command Register(0x16)" },
		{ "AVB Data Register(0x17)" },
	...
	MSD_DBG_INFO(("Topaz_gsysGlobal2RegDump Called.\n"));

	MSG(("\n------------------------------------------------\n"));
	for (i = 0; i < 32; i++)
	{
		retVal = msdGetAnyReg(dev->devNum, TOPAZ_GLOBAL2_DEV_ADDR, i, &data);
		if (retVal != MSD_OK)
		{
			MSD_DBG_ERROR(("msdGetAnyReg returned: %s.\n", msdDisplayStatus(retVal)));
			return retVal;
	...
	MSD_DBG_INFO(("Topaz_gsysGlobal2RegDump Exit.\n"));
	return MSD_OK;
}
