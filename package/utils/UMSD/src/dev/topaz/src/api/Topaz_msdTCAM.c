...
/*
 *  typedef: struct TOPAZ_MSD_TCAM_PG_DATA
 *
...
} TOPAZ_MSD_TCAM_PG_DATA;

/*
 *  typedef: struct TOPAZ_MSD_TCAM_DATA_HW
 *
...
{
	/* Pg0 registers */
	TOPAZ_MSD_TCAM_PG_DATA pg0;
	/* Pg1 registers */
	TOPAZ_MSD_TCAM_PG_DATA pg1;
	/* Pg2 registers */
	TOPAZ_MSD_TCAM_PG_DATA pg2;
} TOPAZ_MSD_TCAM_DATA_HW;

typedef struct
{
    MSD_U32    tcamEntry;
    TOPAZ_MSD_TCAM_DATA_HW    tcamDataP; 
} TOPAZ_MSD_TCAM_OP_DATA;

typedef enum
{
    TOPAZ_TCAM_FLUSH_ALL       = 0x1,
    TOPAZ_TCAM_FLUSH_ENTRY     = 0x2,
    TOPAZ_TCAM_LOAD_ENTRY      = 0x3,
	TOPAZ_TCAM_PURGE_ENTRY	 = 0x3,
    TOPAZ_TCAM_GET_NEXT_ENTRY  = 0x4,
    TOPAZ_TCAM_READ_ENTRY      = 0x5
} TOPAZ_MSD_TCAM_OPERATION;

/****************************************************************************/
/* TCAM operation function declaration.                                    */
/****************************************************************************/
static MSD_STATUS Topaz_tcamOperationPerform
(
    IN   MSD_QD_DEV             *dev,
    IN   TOPAZ_MSD_TCAM_OPERATION    tcamOp,
    INOUT TOPAZ_MSD_TCAM_OP_DATA    *opData
);
static MSD_STATUS Topaz_setTcamHWData
(
	IN	TOPAZ_MSD_TCAM_DATA	*iData,
	OUT	TOPAZ_MSD_TCAM_DATA_HW	*oData
);
static MSD_STATUS Topaz_getTcamHWData
(
	IN	TOPAZ_MSD_TCAM_DATA_HW	*iData,
	OUT	TOPAZ_MSD_TCAM_DATA	*oData
);
static void displayTcamData(TOPAZ_MSD_TCAM_DATA *tcamData);
static void mappingIpv4ToTcam(TOPAZ_MSD_TCAM_DATA *tcamData, MSD_TCAM_ENT_IPV4_UNT *pIpv4Data, MSD_TCAM_ENT_IPV4_UNT *pIpv4Mask);
static void mappingUdpToTcam(int startIndex, TOPAZ_MSD_TCAM_DATA *tcamData, MSD_TCAM_ENT_UDP_UNT *pUdpData, MSD_TCAM_ENT_UDP_UNT *pUdpMask);
static void mappingActionToTcam(TOPAZ_MSD_TCAM_DATA *tcamData, TOPAZ_MSD_TCAM_ENT_ACT *actionPtr);
static void mappingIpv6ToTcam(TOPAZ_MSD_TCAM_DATA *tcamData, TOPAZ_MSD_TCAM_DATA *tcamData2, MSD_TCAM_ENT_IPV6_UNT *pIpv6, MSD_TCAM_ENT_IPV6_UNT *pIpv6Mask);
static void mappingTcpOverIpv4ToTcam(TOPAZ_MSD_TCAM_DATA *tcamData, TOPAZ_MSD_TCAM_DATA *tcamData2, MSD_TCAM_ENT_TCP_UNT *pTcpData, MSD_TCAM_ENT_TCP_UNT *pTcpMask);
static void mappingTcpOverIpv6ToTcam(TOPAZ_MSD_TCAM_DATA *tcamData2, MSD_TCAM_ENT_TCP_UNT *pTcpData, MSD_TCAM_ENT_TCP_UNT *pTcpMask);
static MSD_BOOL checkEntry2Used(TOPAZ_MSD_TCAM_DATA *tcamData2);
/*******************************************************************************
* Topaz_gtcamFlushAll
*
* DESCRIPTION:
...
{
    MSD_STATUS           retVal;
    TOPAZ_MSD_TCAM_OPERATION    op;
    TOPAZ_MSD_TCAM_OP_DATA     tcamOpData;

    MSD_DBG_INFO(("Topaz_gtcamFlushAll Called.\n"));

    /* Program Tuning register */
    op = TOPAZ_TCAM_FLUSH_ALL;
    retVal = Topaz_tcamOperationPerform(dev,op, &tcamOpData);
    if(retVal != MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Topaz_gtcamFlushAll Exit.\n"));
    return MSD_OK;
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gtcamFlushEntry Called.\n"));

    /* check if the given pointer is valid */
	if (tcamPointer >= TOPAZ_MAX_IGR_TCAM_ENTRY)
	{
		MSD_DBG_ERROR(("Bad tcamPointer %u. It should be within [0, %d].\n", (unsigned int)tcamPointer, (int)(TOPAZ_MAX_IGR_TCAM_ENTRY - 1)));
		return MSD_BAD_PARAM;
	}

    /* Program Tuning register */
    op = TOPAZ_TCAM_FLUSH_ENTRY;
    ...
    MSD_DBG_INFO(("Topaz_gtcamFlushEntry Exit.\n"));
    return MSD_OK;
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gtcamLoadEntry Called.\n"));

    /* check if the given pointer is valid */
	if ((tcamPointer >= TOPAZ_MAX_IGR_TCAM_ENTRY) || (tcamData == NULL))
	{
    ...
    /* Program Tuning register */
    op = TOPAZ_TCAM_LOAD_ENTRY;
    tcamOpData.tcamEntry = tcamPointer;
    /* tcamOpData.tcamDataP = tcamData;*/
	Topaz_setTcamHWData(tcamData,&tcamOpData.tcamDataP);
    retVal = Topaz_tcamOperationPerform(dev,op, &tcamOpData);
    if(retVal != MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Topaz_gtcamLoadEntry Exit.\n"));
    return MSD_OK;
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gtcamReadTCAMData Called.\n"));
	...
	Topaz_getTcamHWData(&tcamOpData.tcamDataP,tcamData);

    MSD_DBG_INFO(("Topaz_gtcamReadTCAMData Exit.\n"));
    return MSD_OK;
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gtcamGetNextTCAMData Called.\n"));

    /* check if the given pointer is valid */
	if (tcamPointer == NULL || *tcamPointer >= TOPAZ_MAX_IGR_TCAM_ENTRY || tcamData == NULL)
	{
    ...
    if(retVal == MSD_NO_SUCH)
	{
		...
		MSD_DBG_INFO(("Topaz_gtcamGetNextTCAMData Exit.\n"));
		return retVal;
	}	
...
)
{
	...
	MSD_DBG_INFO(("Topaz_gtcamFindEntry Called.\n"));

	/* check if the given pointer is valid */
	if (tcamPointer >= TOPAZ_MAX_IGR_TCAM_ENTRY || tcamData == NULL || found == NULL)
	{
	...
	*found = MSD_FALSE;
	...
	/* Program Tuning register */
	op = TOPAZ_TCAM_GET_NEXT_ENTRY;
	tcamOpData.tcamEntry = (tcamPointer == 0 ? 0x7f : (tcamPointer - 1));
	retVal = Topaz_tcamOperationPerform(dev, op, &tcamOpData);
	if (retVal == MSD_NO_SUCH)
	{
		MSD_DBG_INFO(("No higher valid entry founded return MSD_NO_SUCH"));
		MSD_DBG_INFO(("Topaz_gtcamFindEntry Exit.\n"));
		return MSD_OK;
	}
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gtcamSetMode Called.\n"));
    ...
    if (mode < TOPAZ_MSD_TCAM_MODE_DISABLE || mode > TOPAZ_MSD_TCAM_MODE_ENABLE_96)
    {
    ...
    MSD_DBG_INFO(("Topaz_gtcamSetMode Exit.\n"));
    return MSD_OK;
}
/*******************************************************************************
* Topaz_gtcamGetMode
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gtcamGetMode Called.\n"));
    ...
    *mode = data;
    ...
    MSD_DBG_INFO(("Topaz_gtcamGetMode Exit.\n"));
    return MSD_OK;
}
/*******************************************************************************
...
)
{
    MSD_STATUS retVal;
    MSD_BOOL found;
    TOPAZ_MSD_TCAM_DATA  tcamData;
    MSD_U8 hwPort;         /* the physical port number     */

    MSD_DBG_INFO(("Topaz_gtcamAddEntryPort Called.\n"));
    ...
    retVal = Topaz_gtcamFindEntry(dev, tcamPointer, &tcamData, &found);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Topaz_gtcamFindEntry in Topaz_gtcamAddEntryPort returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    retVal = Topaz_gtcamLoadEntry(dev, tcamPointer, &tcamData);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Topaz_gtcamLoadEntry in Topaz_gtcamAddEntryPort returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Topaz_gtcamAddEntryPort Exit.\n"));
    return retVal;
}
/*******************************************************************************
...
)
{
    ...
    MSD_DBG_INFO(("Topaz_gtcamRemoveEntryPort Called.\n"));
    ...
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Topaz_gtcamFindEntry in Topaz_gtcamRemoveEntryPort returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Topaz_gtcamLoadEntry in Topaz_gtcamRemoveEntryPort returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Topaz_gtcamRemoveEntryPort Exit.\n"));
    return retVal;
}
...
/* Internal functions.                                                  */
/****************************************************************************/
#define Topaz_BIT(n)                  ( 1<<(n) )
#define Topaz_BIT_SET(y, mask)        ( y |=  (mask) )
#define Topaz_BIT_CLEAR(y, mask)      ( y &= ~(mask) )
#define Topaz_BIT_FLIP(y, mask)       ( y ^=  (mask) )

/* ! Create a bitmask of length \a len. */
#define Topaz_BIT_MASK(len)           ( Topaz_BIT(len)-1 )
/* ! Create a bitfield mask of length \a starting at bit \a start. */
#define Topaz_BF_MASK(start, len)     ( Topaz_BIT_MASK(len)<<(start) )
/* ! Prepare a bitmask for insertion or combining. */
#define Topaz_BF_PREP(x, start, len)  ( ((x)&Topaz_BIT_MASK(len)) << (start) )
/* ! Extract a bitfield of length \a len starting at bit \a start from \a y. */
#define Topaz_BF_GET(y, start, len)   ( ((y)>>(start)) & Topaz_BIT_MASK(len) )
/* ! Insert a new bitfield value \a x into \a y. */
#define Topaz_BF_SET(y, x, start, len)    \
    ( y= ((y) &~ Topaz_BF_MASK(start, len)) | Topaz_BF_PREP(x, start, len) )
...
)
{
	...
	msdMemSet(oData,0,sizeof(TOPAZ_MSD_TCAM_DATA_HW));

	/*key*/
	Topaz_BF_SET(oData->pg0.frame[0],iData->frameTypeMask, 14, 2);
	Topaz_BF_SET(oData->pg0.frame[0],iData->frameType, 6, 2);

	Topaz_BF_SET(oData->pg0.frame[1],(iData->spvMask&0x7f), 8, 7);
	Topaz_BF_SET(oData->pg0.frame[1],(iData->spv&0x7f), 0, 7);

	Topaz_BF_SET(oData->pg0.frame[2],iData->ppriMask, 12, 4);
	Topaz_BF_SET(oData->pg0.frame[2],iData->ppri, 4, 4);
	Topaz_BF_SET(oData->pg0.frame[2],((iData->pvidMask&0xf00)>>8), 8, 4);
	Topaz_BF_SET(oData->pg0.frame[2],((iData->pvid&0xf00)>>8), 0, 4);

	Topaz_BF_SET(oData->pg0.frame[3],(iData->pvidMask&0xff), 8, 8);
	Topaz_BF_SET(oData->pg0.frame[3],(iData->pvid&0xff), 0, 8);

	/*Page 0 (0~21) bytes*/
	for(i=4; i<26; i++)
	{
		Topaz_BF_SET(oData->pg0.frame[i],iData->frameOctetMask[i-4],8,8);
		Topaz_BF_SET(oData->pg0.frame[i],iData->frameOctet[i-4],0,8);
	}

	/*Page 1 (22~47) bytes*/
	for(i=0; i<26; i++)
	{
		Topaz_BF_SET(oData->pg1.frame[i],iData->frameOctetMask[22+i],8,8);
		Topaz_BF_SET(oData->pg1.frame[i],iData->frameOctet[22+i],0,8);
	}

	/*Action*/
	Topaz_BF_SET(oData->pg2.frame[0],iData->continu, 15, 1);
	Topaz_BF_SET(oData->pg2.frame[0],iData->interrupt, 14, 1);
	Topaz_BF_SET(oData->pg2.frame[0],iData->IncTcamCtr, 13, 1);
	Topaz_BF_SET(oData->pg2.frame[0],(MSD_U16)(iData->vidOverride), 12, 1);
	Topaz_BF_SET(oData->pg2.frame[0],iData->vidData, 0, 12);

	Topaz_BF_SET(oData->pg2.frame[1],iData->nextId, 8, 8);
	Topaz_BF_SET(oData->pg2.frame[1],(MSD_U16)(iData->qpriOverride), 7, 1);
	Topaz_BF_SET(oData->pg2.frame[1],iData->qpriData, 4, 2);
	Topaz_BF_SET(oData->pg2.frame[1],(MSD_U16)(iData->fpriOverride), 3, 1);
	Topaz_BF_SET(oData->pg2.frame[1],iData->fpriData, 0, 3);

	Topaz_BF_SET(oData->pg2.frame[2],iData->dpvSF, 11, 1);
	Topaz_BF_SET(oData->pg2.frame[2],(MSD_U16)iData->dpvData, 0, 7);

	/*
	Topaz_BF_SET(oData->pg2.frame[3],(MSD_U16)0, 0, 16);
	*/
	oData->pg2.frame[3] = 0;

	Topaz_BF_SET(oData->pg2.frame[4],iData->dpvMode, 14, 2);
	Topaz_BF_SET(oData->pg2.frame[4],iData->unKnownFilter, 8, 2);


	Topaz_BF_SET(oData->pg2.frame[5],(MSD_U16)(iData->ldBalanceOverride), 15, 1);
	Topaz_BF_SET(oData->pg2.frame[5],iData->ldBalanceData, 12, 3);

	Topaz_BF_SET(oData->pg2.frame[6],(MSD_U16)(iData->factionOverride), 15, 1);
	Topaz_BF_SET(oData->pg2.frame[6],iData->factionData, 0, 15);
...
)
{
	int i;
	/*key*/
	oData->frameTypeMask = (MSD_U8)(Topaz_BF_GET(iData->pg0.frame[0],14, 2));
	oData->frameType = (MSD_U8)(Topaz_BF_GET(iData->pg0.frame[0],6, 2));

    oData->spvMask = Topaz_BF_GET(iData->pg0.frame[1],8, 7);
	oData->spv = Topaz_BF_GET(iData->pg0.frame[1], 0, 7);

	oData->ppriMask = (MSD_U8)(Topaz_BF_GET(iData->pg0.frame[2],12, 4));
	oData->ppri = (MSD_U8)(Topaz_BF_GET(iData->pg0.frame[2],4, 4));

	oData->pvidMask = (Topaz_BF_GET(iData->pg0.frame[2], 8, 4) << 8) | Topaz_BF_GET(iData->pg0.frame[3],8, 8);

	oData->pvid = (Topaz_BF_GET(iData->pg0.frame[2], 0, 4) << 8) | Topaz_BF_GET(iData->pg0.frame[3], 0, 8);

	/*Page 0 (0~21) bytes*/
	for(i=4; i<26; i++)
	{
		oData->frameOctetMask[i-4] = (MSD_U8)(Topaz_BF_GET(iData->pg0.frame[i],8, 8));
		oData->frameOctet[i-4] = (MSD_U8)(Topaz_BF_GET(iData->pg0.frame[i],0,8));
	}

	/*Page 1 (22~47) bytes*/
	for(i=0; i<26; i++)
	{
		oData->frameOctetMask[22+i] = (MSD_U8)(Topaz_BF_GET(iData->pg1.frame[i],8,8));
		oData->frameOctet[22+i] = (MSD_U8)(Topaz_BF_GET(iData->pg1.frame[i],0,8));
	}

	/*Action*/
	oData->continu = (MSD_U8)(Topaz_BF_GET(iData->pg2.frame[0], 15, 1));
	oData->interrupt = (MSD_U8)(Topaz_BF_GET(iData->pg2.frame[0], 14, 1));
	oData->IncTcamCtr = (MSD_U8)(Topaz_BF_GET(iData->pg2.frame[0], 13, 1));
	oData->vidOverride = Topaz_BF_GET(iData->pg2.frame[0], 12, 1);
	oData->vidData = Topaz_BF_GET(iData->pg2.frame[0], 0, 12);

	oData->nextId = (MSD_U8)(Topaz_BF_GET(iData->pg2.frame[1], 8, 8));
	oData->qpriOverride = Topaz_BF_GET(iData->pg2.frame[1], 7, 1);
	oData->qpriData = (MSD_U8)(Topaz_BF_GET(iData->pg2.frame[1], 4, 2));
	oData->fpriOverride = Topaz_BF_GET(iData->pg2.frame[1], 3, 1);
	oData->fpriData = (MSD_U8)(Topaz_BF_GET(iData->pg2.frame[1], 0, 3));

	oData->dpvSF = (MSD_U8)(Topaz_BF_GET(iData->pg2.frame[2], 11, 1));
	oData->dpvData = Topaz_BF_GET(iData->pg2.frame[2], 0, 7);

	oData->dpvMode = (MSD_U8)(Topaz_BF_GET(iData->pg2.frame[4], 14, 2));
	oData->unKnownFilter = (MSD_U8)(Topaz_BF_GET(iData->pg2.frame[4], 8, 2));

	oData->ldBalanceOverride = Topaz_BF_GET(iData->pg2.frame[5], 15, 1);
	oData->ldBalanceData = (MSD_U8)(Topaz_BF_GET(iData->pg2.frame[5], 12, 3));

	oData->factionOverride = Topaz_BF_GET(iData->pg2.frame[6], 15, 1);
	oData->factionData = Topaz_BF_GET(iData->pg2.frame[6], 0, 15);

	return MSD_OK;
}
static MSD_STATUS Topaz_tcamSetPage0Data(MSD_QD_DEV *dev, TOPAZ_MSD_TCAM_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<0x1c; i++)
	{
		retVal = msdSetAnyReg(dev->devNum, TOPAZ_TCAM_DEV_ADDR,(MSD_U8)i,tcamDataP->pg0.frame[i-2]);
		if(retVal != MSD_OK)
		{
			return retVal;
		}
...
static MSD_STATUS tcamSetPage1Data(MSD_QD_DEV *dev, TOPAZ_MSD_TCAM_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<0x1c; i++)
	{
		retVal = msdSetAnyReg(dev->devNum, TOPAZ_TCAM_DEV_ADDR,(MSD_U8)i,tcamDataP->pg1.frame[i-2]);
		if(retVal != MSD_OK)
		{
			return retVal;
		}
...
static MSD_STATUS tcamSetPage2Data(MSD_QD_DEV *dev, TOPAZ_MSD_TCAM_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<9; i++)
	{
		retVal = msdSetAnyReg(dev->devNum, TOPAZ_TCAM_DEV_ADDR,(MSD_U8)i,tcamDataP->pg2.frame[i-2]);
		if(retVal != MSD_OK)
		{
			return retVal;
		}
...
static MSD_STATUS Topaz_tcamGetPage0Data(MSD_QD_DEV *dev, TOPAZ_MSD_TCAM_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<0x1c; i++)
	{
		retVal = msdGetAnyReg(dev->devNum, TOPAZ_TCAM_DEV_ADDR,(MSD_U8)i,&tcamDataP->pg0.frame[i-2]);
		if(retVal != MSD_OK)
		{
			return retVal;
		}
...
static MSD_STATUS Topaz_tcamGetPage1Data(MSD_QD_DEV *dev, TOPAZ_MSD_TCAM_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<0x1c; i++)
	{
		retVal = msdGetAnyReg(dev->devNum, TOPAZ_TCAM_DEV_ADDR,(MSD_U8)i,&tcamDataP->pg1.frame[i-2]);
		if(retVal != MSD_OK)
		{
			return retVal;
		}
...
static MSD_STATUS Topaz_tcamGetPage2Data(MSD_QD_DEV *dev, TOPAZ_MSD_TCAM_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<9; i++)
	{
		retVal = msdGetAnyReg(dev->devNum, TOPAZ_TCAM_DEV_ADDR,(MSD_U8)i,&tcamDataP->pg2.frame[i-2]);
		if(retVal != MSD_OK)
		{
			return retVal;
		}
...
static MSD_STATUS Topaz_waitTcamReady(MSD_QD_DEV           *dev)
{
    MSD_STATUS       retVal;    /* Functions return value */
    ...
    while(data == 1)
    {
        retVal = msdGetAnyRegField(dev->devNum, TOPAZ_TCAM_DEV_ADDR,TOPAZ_QD_REG_TCAM_OPERATION,15,1,&data);
        if(retVal != MSD_OK)
        {
            return retVal;
        }
...
)
{
	...
	retVal = Topaz_waitTcamReady(dev);
	if(retVal != MSD_OK)
	{
	...
	switch (tcamOp)
	{
		case TOPAZ_TCAM_FLUSH_ALL:
		{
			...
			retVal = msdSetAnyReg(dev->devNum, TOPAZ_TCAM_DEV_ADDR,TOPAZ_QD_REG_TCAM_OPERATION,data);
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
				return retVal;
		...
		case TOPAZ_TCAM_LOAD_ENTRY:
		/*    case TOPAZ_TCAM_PURGE_ENTRY: */
		{
			/* load Page 2 */
			/* Wait until the tcam in ready. */
			retVal = Topaz_waitTcamReady(dev);
			...
			data = 0;
			data = (MSD_U16)((1 << 15) | (TOPAZ_TCAM_LOAD_ENTRY << 12) | (2 << 10) | opData->tcamEntry);
			retVal = msdSetAnyReg(dev->devNum,  TOPAZ_TCAM_DEV_ADDR, TOPAZ_QD_REG_TCAM_OPERATION, data);
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
			...
			data = 0;
			data = (MSD_U16)((1 << 15) | (TOPAZ_TCAM_LOAD_ENTRY << 12) | (1 << 10) | opData->tcamEntry);
			retVal = msdSetAnyReg(dev->devNum, TOPAZ_TCAM_DEV_ADDR,TOPAZ_QD_REG_TCAM_OPERATION,data);
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
			...
			retVal = Topaz_tcamSetPage0Data(dev, &opData->tcamDataP);
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
				return retVal;
			...
			data = 0;
			data = (MSD_U16)((1 << 15) | (TOPAZ_TCAM_LOAD_ENTRY << 12) | (0 << 10) | opData->tcamEntry);
			retVal = msdSetAnyReg(dev->devNum, TOPAZ_TCAM_DEV_ADDR,TOPAZ_QD_REG_TCAM_OPERATION,data);
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
		...
		case TOPAZ_TCAM_GET_NEXT_ENTRY:
		{
			retVal = Topaz_waitTcamReady(dev);
			if(retVal != MSD_OK)
			{
			...
			retVal = msdGetAnyReg(dev->devNum, TOPAZ_TCAM_DEV_ADDR,TOPAZ_QD_REG_TCAM_OPERATION, &data);
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
				return retVal;
			...
			{
			MSD_U16 data1;
			retVal = msdGetAnyReg(dev->devNum, TOPAZ_TCAM_DEV_ADDR,TOPAZ_QD_REG_TCAM_P0_KEYS_1, &data1);
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
				return retVal;
			...
			opData->tcamEntry = data&0x7f;

		}
		case TOPAZ_TCAM_READ_ENTRY:
		{
			tcamOp = TOPAZ_TCAM_READ_ENTRY;
			...
			retVal = Topaz_tcamGetPage0Data(dev, &opData->tcamDataP);
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
				return retVal;
			...
			retVal = Topaz_tcamGetPage1Data(dev, &opData->tcamDataP);
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
				return retVal;
			...
			retVal = Topaz_tcamGetPage2Data(dev, &opData->tcamDataP);
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
				return retVal;
...
)
{
	...
	while ((status = Topaz_gtcamGetNextTCAMData(dev, &tcamPointer, &tcamData)) == MSD_OK)
	{
...
)
{
    MSD_STATUS retVal;
    TOPAZ_MSD_TCAM_DATA  tcamData;
    TOPAZ_MSD_TCAM_DATA  tcamData2;
    int i;

    MSD_DBG_INFO(("Topaz_gtcamAdvConfig Called.\n"));

    msdMemSet(&tcamData, 0, sizeof(TOPAZ_MSD_TCAM_DATA));
    msdMemSet(&tcamData2, 0, sizeof(TOPAZ_MSD_TCAM_DATA));
    ...
    if (checkEntry2Used(&tcamData2) == MSD_TRUE)
    {
        ...
        retVal = Topaz_gtcamLoadEntry(dev, tcamPointer2, &tcamData2);
        if (retVal != MSD_OK)
        {
    ...
    MSD_DBG_INFO(("Topaz_gtcamAdvConfig Exit.\n"));
    return MSD_OK;
}
