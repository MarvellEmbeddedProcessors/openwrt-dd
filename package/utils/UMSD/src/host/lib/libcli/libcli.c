...
#define fprintf(stream, format, ...) _fprintf(stream, format, ##__VA_ARGS__)
/*
...
struct cli_filter_cmds filter_cmds_list[] = 
{
    {NULL, NULL}
};
static struct cli_filter_cmds *filter_cmds = filter_cmds_list;
void cli_set_filter(struct cli_filter_cmds *commands)
{
    filter_cmds = commands;
}
...
/*     cli_register_command(cli, 0, "quit", cli_int_quit, PRIVILEGE_UNPRIVILEGED, MODE_ANY, "Disconnect");
    ...
    cli_register_command(cli, 0, "exit", cli_int_exit, PRIVILEGE_UNPRIVILEGED, MODE_ANY, "Exit from current program");
...
static int cli_get_completions(struct cli_def *cli, const char *command, char **completions, int max_completions)
{
    ...
    if (filter) // complete filters
    {
        ...
        for (i = 0; filter_cmds[i].cmd && k < max_completions; i++)
        {
			...
			{
				if(strncasecmp(filter_cmds[i].help, command, strlen(filter_cmds[i].help)) == 0)
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            if (c == '\r')
            {
                ...
                {
                    if (cli->state != STATE_PASSWORD && cli->state != STATE_ENABLE_PASSWORD)
                        _write(sockfd, &cmd[tempCursor], 1);
            ...
            if (c == CTRL('I'))
            {
                ...
                else
                {
					...
					int i, j;
					char temp[CLI_MAX_LINE_WORDS] = {0};
					int flag = 0;
					for(i = 0; i < CLI_MAX_LINE_WORDS; i ++)
					{
						char c = completions[0][i];
						for(j = 1; j < num_completions; j++)
						{
					...
					//if(strstr(cmd, temp) == 0)
					{
						...
						//cmd[l++] = ' ';
						cursor = l;
						_write(sockfd, temp, strlen(temp));
						//_write(sockfd, " ", 1);
					}
					//else
					//{						
...
#ifdef WIN32
    _pclose(cli->client);
    //CloseHandle((HANDLE)_get_osfhandle(fileno(cli->client)));
#else
    fclose(cli->client);
#endif
    cli->client = 0;
